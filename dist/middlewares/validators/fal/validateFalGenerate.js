...
exports.validateFalGenerate = [
    ...
    (0, express_validator_1.body)('generationType').optional().isIn(['text-to-image', 'logo', 'sticker-generation', 'text-to-video', 'text-to-music', 'mockup-generation', 'product-generation', 'ad-generation', 'live-chat', 'text-to-character']).withMessage('invalid generationType'),
    ...
    (0, express_validator_1.body)('num_images').optional().isInt({ min: 1, max: 10 }),
...
exports.validateFalOutpaint = [
    (0, express_validator_1.body)('image_url').optional().isString().notEmpty(),
    (0, express_validator_1.body)('image').optional().isString().notEmpty(),
    (0, express_validator_1.body)('expand_left').optional().isInt({ min: 0, max: 700 }),
    (0, express_validator_1.body)('expand_right').optional().isInt({ min: 0, max: 700 }),
    (0, express_validator_1.body)('expand_top').optional().isInt({ min: 0, max: 700 }),
    (0, express_validator_1.body)('expand_bottom').optional().isInt({ min: 0, max: 700 }),
    (0, express_validator_1.body)('zoom_out_percentage').optional().isFloat({ min: 0, max: 100 }),
    (0, express_validator_1.body)('prompt').optional().isString(),
    (0, express_validator_1.body)('num_images').optional().isInt({ min: 1, max: 4 }),
    (0, express_validator_1.body)('enable_safety_checker').optional().isBoolean(),
    (0, express_validator_1.body)('sync_mode').optional().isBoolean(),
    (0, express_validator_1.body)('output_format').optional().isIn(['png', 'jpeg', 'jpg', 'webp']),
    (0, express_validator_1.body)('aspect_ratio').optional().isIn(['1:1', '16:9', '9:16', '4:3', '3:4']),
    (req, _res, next) => {
        const hasUrl = typeof req.body?.image_url === 'string' && req.body.image_url.length > 0;
        const hasImage = typeof req.body?.image === 'string' && req.body.image.length > 0;
        if (!hasUrl && !hasImage)
...
exports.validateFalBriaExpand = [
    (0, express_validator_1.body)('image_url').optional().isString().notEmpty(),
    (0, express_validator_1.body)('image').optional().isString().notEmpty(),
    (0, express_validator_1.body)('canvas_size').optional().isArray({ min: 2, max: 2 }),
    (0, express_validator_1.body)('canvas_size.*').optional().isInt({ min: 1, max: 5000 }),
    (0, express_validator_1.body)('aspect_ratio').optional().isIn(['1:1', '2:3', '3:2', '3:4', '4:3', '4:5', '5:4', '9:16', '16:9']),
    (0, express_validator_1.body)('original_image_size').optional().isArray({ min: 2, max: 2 }),
    (0, express_validator_1.body)('original_image_size.*').optional().isInt({ min: 1, max: 5000 }),
    (0, express_validator_1.body)('original_image_location').optional().isArray({ min: 2, max: 2 }),
    (0, express_validator_1.body)('original_image_location.*').optional().isInt({ min: -10000, max: 10000 }),
    (0, express_validator_1.body)('prompt').optional().isString(),
    (0, express_validator_1.body)('seed').optional().isInt(),
    (0, express_validator_1.body)('negative_prompt').optional().isString(),
    (0, express_validator_1.body)('sync_mode').optional().isBoolean(),
    (req, _res, next) => {
        ...
        if (Array.isArray(req.body?.canvas_size) && req.body.canvas_size.length === 2) {
            ...
            if (!isFinite(w) || !isFinite(h) || w <= 0 || h <= 0 || w > 5000 || h > 5000) {
                return next(new errorHandler_1.ApiError('canvas_size must be [width,height] each <= 5000', 400));
            }
        }
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty())
...
exports.validateFalBriaGenfill = [
    (0, express_validator_1.body)('image_url').optional().isString().notEmpty(),
    (0, express_validator_1.body)('image').optional().isString().notEmpty(),
    (0, express_validator_1.body)('mask_url').optional().isString().notEmpty(),
    (0, express_validator_1.body)('mask').optional().isString().notEmpty(),
    (0, express_validator_1.body)('prompt').isString().notEmpty(),
    (0, express_validator_1.body)('negative_prompt').optional().isString(),
    (0, express_validator_1.body)('seed').optional().isInt(),
    (0, express_validator_1.body)('num_images').optional().isInt({ min: 1, max: 4 }),
    ...
    (req, _res, next) => {
        ...
        if (!hasMaskUrl && !hasMask) {
            return next(new errorHandler_1.ApiError('mask_url or mask is required', 400));
        }
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty())
            return next(new errorHandler_1.ApiError('Validation failed', 400, errors.array()));
    ...
    async (req, _res, next) => {
        ...
        if (typeof url === 'string' && url.length > 0) {
            try {
                ...
                return next(new errorHandler_1.ApiError('Failed to validate image URL for Topaz upscale', 400));
            }
        }
            ...
            return next(new errorHandler_1.ApiError('image_url or data URI image is required', 400));
        }
        next();
    }
];
