...
async function computeMinimaxVideoCost(req) {
    ...
    if (model === 'MiniMax-Hailuo-02') {
        ...
        if (res === '768P' && dur === '6')
            display = `Minimax-Hailuo-2.3${fastPrefix} 768P 6s`;
        else if (res === '768P' && dur === '10')
            display = `Minimax-Hailuo-2.3${fastPrefix} 768P 10s`;
        else if (res === '1080P' && dur === '6')
            display = `Minimax-Hailuo-2.3${fastPrefix} 1080P 6s`;
    }
    ...
    if ((model === 'MiniMax-Hailuo-2.3' || model === 'MiniMax-Hailuo-2.3-Fast') && base == null) {
        ...
        const res = String(resolution || '').toUpperCase();
        if (res === '768P' && dur === '6')
            base = findCreditsExact('Minimax-Hailuo-02 768P 6s');
        else if (res === '768P' && dur === '10')
            base = findCreditsExact('Minimax-Hailuo-02 768P 10s');
        else if (res === '1080P' && dur === '6')
            base = findCreditsExact('Minimax-Hailuo-02 1080P 6s');
    }
    if (base == null)
