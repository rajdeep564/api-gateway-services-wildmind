...
exports.computeFalOutpaintCost = computeFalOutpaintCost;
...
exports.computeFalBriaGenfillCost = computeFalBriaGenfillCost;
...
const zataUpload_1 = require("../storage/zataUpload");
...
async function computeFalOutpaintCost(req) {
    const body = req.body || {};
    let url = typeof body.image_url === 'string' && body.image_url.length > 0 ? body.image_url : undefined;
    if (!url && typeof body.image === 'string' && body.image.startsWith('data:')) {
        try {
            const uid = req?.uid || 'anon';
            const stored = await (0, zataUpload_1.uploadDataUriToZata)({ dataUri: body.image, keyPrefix: `users/${uid}/pricing/outpaint/${Date.now()}`, fileName: 'source' });
    ...
    const totalMegapixels = (outputWidth * outputHeight * numImages) / 1000000;
...
async function computeFalBriaGenfillCost(req) {
    const display = 'fal-ai/bria/genfill';
    const base = findCredits(display);
    if (base == null)
        throw new Error('Unsupported FAL bria/genfill pricing');
    const body = req.body || {};
    ...
    return { cost: Math.ceil(base * count), pricingVersion: exports.FAL_PRICING_VERSION, meta: { model: display, num_images: count } };
}
...
async function computeFalTopazUpscaleImageCost(req) {
    ...
    if (!url && typeof body.image === 'string' && body.image.startsWith('data:')) {
        try {
            const uid = req?.uid || 'anon';
            const stored = await (0, zataUpload_1.uploadDataUriToZata)({ dataUri: body.image, keyPrefix: `users/${uid}/pricing/topaz/${Date.now()}`, fileName: 'source' });
            url = stored.publicUrl;
        }
        catch {
            url = undefined;
