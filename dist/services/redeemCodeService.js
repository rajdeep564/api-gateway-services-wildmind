...
exports.createRedeemCodes = createRedeemCodes;
exports.validateAndUseRedeemCode = validateAndUseRedeemCode;
exports.getRedeemCodeInfo = getRedeemCodeInfo;
const redeemCodeRepository_1 = require("../repository/redeemCodeRepository");
...
async function createRedeemCodes(request) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        try {
            await redeemCodeRepository_1.redeemCodeRepository.createRedeemCode(code, type, planCode, maxUsesPerCode, validUntilDate);
            codes.push(code);
        }
        catch (error) {
...
async function validateAndUseRedeemCode(code, uid, username, email) {
    // Validate the redeem code
    const validation = await redeemCodeRepository_1.redeemCodeRepository.validateRedeemCode(code, uid);
    if (!validation.valid || !validation.redeemCode) {
        throw new errorHandler_1.ApiError(validation.error || 'Invalid redeem code', 400);
    }
    const { redeemCode } = validation;
    const planCode = redeemCode.planCode;
    // Get credits for the plan
    const creditsToGrant = planCode === 'PLAN_A' ? creditDistribution_1.PLAN_CREDITS.PLAN_A : creditDistribution_1.PLAN_CREDITS.PLAN_B;
    try {
        // Switch user to the new plan (this will grant credits and set plan)
        await creditsService_1.creditsService.switchPlan(uid, planCode);
        // Record the redeem code usage
        await redeemCodeRepository_1.redeemCodeRepository.useRedeemCode(code, uid, username, email, planCode, creditsToGrant);
        return { planCode, creditsGranted: creditsToGrant };
    }
    catch (error) {
        console.error('Failed to apply redeem code:', error);
        throw new errorHandler_1.ApiError('Failed to apply redeem code. Please try again.', 500);
    }
}
async function getRedeemCodeInfo(code) {
    const redeemCode = await redeemCodeRepository_1.redeemCodeRepository.getRedeemCode(code);
    if (!redeemCode) {
    ...
    const creditsToGrant = redeemCode.planCode === 'PLAN_A' ? creditDistribution_1.PLAN_CREDITS.PLAN_A : creditDistribution_1.PLAN_CREDITS.PLAN_B;
