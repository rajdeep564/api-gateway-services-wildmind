...
exports.replicateQueueStatus = replicateQueueStatus;
exports.replicateQueueResult = replicateQueueResult;
...
async function removeBackground(uid, body) {
    ...
    if (modelBase.startsWith('bria/eraser')) {
        ...
        if (!input.image && typeof anyBody.image_url === 'string' && anyBody.image_url.length > 0)
        ...
        if (anyBody.mask_type)
            input.mask_type = String(anyBody.mask_type).toLowerCase() === 'manual' ? 'manual' : (String(anyBody.mask_type).toLowerCase() === 'automatic' ? 'automatic' : undefined);
        ...
        if (typeof anyBody.sync === 'boolean')
            input.sync = anyBody.sync;
        else
            input.sync = true;
    }
    else {
    ...
    try {
        ...
        let output = await replicate.run(modelSpec, { input });
        // eslint-disable-next-line no-console
    ...
    try {
        ...
        if (modelBase === 'bytedance/seedream-4') {
            ...
            if (images.length > 0) {
                ...
                for (let i = 0; i < images.length; i++) {
                    ...
                    try {
                        if (typeof img === 'string' && img.startsWith('data:')) {
                            const uploaded = await (0, zataUpload_1.uploadDataUriToZata)({ dataUri: img, keyPrefix: `users/${username}/input/${historyId}`, fileName: `seedream-ref-${i + 1}` });
                ...
                async function normalizeIfNeeded(url, idx) {
                    try {
                        let buf = null;
                        ...
                        if (ratio > maxR) {
                            ...
                            const padded = await (0, sharp_1.default)(buf).extend({ top, bottom, left: 0, right: 0, background: { r: 0, g: 0, b: 0 } }).toBuffer();
                            const uploaded = await (0, zataUpload_1.uploadDataUriToZata)({ dataUri: `data:image/jpeg;base64,${padded.toString('base64')}`, keyPrefix: `users/${username}/input/${historyId}`, fileName: `seedream-ref-fixed-${idx + 1}.jpg` });
                            ...
                            const padded = await (0, sharp_1.default)(buf).extend({ top: 0, bottom: 0, left, right, background: { r: 0, g: 0, b: 0 } }).toBuffer();
                            const uploaded = await (0, zataUpload_1.uploadDataUriToZata)({ dataUri: `data:image/jpeg;base64,${padded.toString('base64')}`, keyPrefix: `users/${username}/input/${historyId}`, fileName: `seedream-ref-fixed-${idx + 1}.jpg` });
                            return uploaded.publicUrl;
                        }
                    }
            ...
            if (requested > 1 && outputUrls.length < requested) {
                ...
                for (let i = 0; i < runsNeeded; i++) {
                    try {
                        ...
                        const more = await replicate.run(modelSpec, { input: rerunInput });
                        ...
                        // eslint-disable-next-line no-console
                        console.error('[seedream] sequential fallback run failed', e?.message || e);
                    }
                    if (outputUrls.length >= requested)
                        break;
                }
            }
        }
...
async function resolveWanModelFast(body) {
    const s = (body?.speed ?? '').toString().toLowerCase();
    const m = (body?.model ?? '').toString().toLowerCase();
    const speedFast = s === 'fast' || s === 'true' || s.includes('fast') || body?.speed === true;
    const modelFast = m.includes('fast');
...
async function getLatestModelVersion(replicate, modelBase) {
    try {
        ...
        const latestVersion = model?.latest_version?.id || (Array.isArray(model?.versions) ? model.versions[0]?.id : null);
...
async function wanT2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit WAN T2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
async function wanI2vSubmit(uid, body) {
    if (!body?.image)
        throw new errorHandler_1.ApiError('image is required', 400);
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit WAN I2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
async function replicateQueueStatus(_uid, requestId) {
    const replicate = ensureReplicate();
    try {
        ...
        throw new errorHandler_1.ApiError(e?.message || 'Failed to fetch Replicate status', 502);
    }
}
async function replicateQueueResult(uid, requestId) {
    const replicate = ensureReplicate();
    try {
        const result = await replicate.predictions.get(requestId);
        const located = await generationHistoryRepository_1.generationHistoryRepository.findByProviderTaskId(uid, 'replicate', requestId);
        if (!located)
            return result;
        ...
        const videoItem = { id: requestId, url: storedUrl, storagePath, originalUrl: outputUrl };
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'completed', videos: [videoItem] });
        try {
            const fresh = await generationHistoryRepository_1.generationHistoryRepository.get(uid, historyId);
            if (fresh)
        ...
        try {
            const fresh = await generationHistoryRepository_1.generationHistoryRepository.get(uid, historyId);
            const model = fresh?.model?.toString().toLowerCase() || '';
            const modeGuess = fresh?.prompt && model.includes('i2v') ? 'i2v' : 't2v';
            if (model.includes('wan-2.5')) {
                const fakeReq = { body: { mode: modeGuess, duration: fresh?.duration, resolution: fresh?.resolution, model: fresh?.model } };
                const { cost, pricingVersion, meta } = await (0, wanPricing_1.computeWanVideoCost)(fakeReq);
                await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, historyId, cost, `replicate.queue.wan-${modeGuess}`, { ...meta, historyId, provider: 'replicate', pricingVersion });
            }
            else if (model.includes('kling-v2.')) {
                const { computeKlingVideoCost } = await Promise.resolve().then(() => __importStar(require('../utils/pricing/klingPricing')));
                const fakeReq = { body: { kind: modeGuess, duration: fresh?.duration, resolution: fresh?.resolution, model: fresh?.model, kling_mode: fresh?.kling_mode } };
                const { cost, pricingVersion, meta } = await computeKlingVideoCost(fakeReq);
                await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, historyId, cost, `replicate.queue.kling-${modeGuess}`, { ...meta, historyId, provider: 'replicate', pricingVersion });
            }
            else if (model.includes('seedance')) {
                const { computeSeedanceVideoCost } = await Promise.resolve().then(() => __importStar(require('../utils/pricing/seedancePricing')));
                const fakeReq = { body: { kind: modeGuess, duration: fresh?.duration, resolution: fresh?.resolution, model: fresh?.model } };
                const { cost, pricingVersion, meta } = await computeSeedanceVideoCost(fakeReq);
                await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, historyId, cost, `replicate.queue.seedance-${modeGuess}`, { ...meta, historyId, provider: 'replicate', pricingVersion });
            }
            else if (model.includes('pixverse')) {
                const { computePixverseVideoCost } = await Promise.resolve().then(() => __importStar(require('../utils/pricing/pixversePricing')));
                const fakeReq = { body: { kind: modeGuess, duration: fresh?.duration, quality: fresh?.quality || fresh?.resolution, model: fresh?.model } };
                const { cost, pricingVersion, meta } = await computePixverseVideoCost(fakeReq);
                await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, historyId, cost, `replicate.queue.pixverse-${modeGuess}`, { ...meta, historyId, provider: 'replicate', pricingVersion });
            }
        }
        catch { }
        return { videos: [videoItem], historyId, model: located.item?.model, requestId, status: 'completed' };
    }
    catch (e) {
        throw new errorHandler_1.ApiError(e?.message || 'Failed to fetch Replicate result', 502);
    }
}
Object.assign(exports.replicateService, { wanT2vSubmit, wanI2vSubmit, replicateQueueStatus, replicateQueueResult });
// ============ Queue-style API for Replicate Kling ============
async function klingT2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    const replicate = ensureReplicate();
    ...
    const input = { prompt: body.prompt, duration: durationSec, aspect_ratio: aspect };
    if (body.guidance_scale != null)
        input.guidance_scale = Math.max(0, Math.min(1, Number(body.guidance_scale)));
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit Kling T2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
async function klingI2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    const hasImg = !!(body?.image || body?.start_image);
    if (!hasImg)
        throw new errorHandler_1.ApiError('image or start_image is required', 400);
    const replicate = ensureReplicate();
    ...
    const input = { prompt: body.prompt, duration: durationSec };
    if (body.image)
        input.image = String(body.image);
    if (body.start_image)
        input.start_image = String(body.start_image);
    if (body.end_image)
        input.end_image = String(body.end_image);
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit Kling I2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
Object.assign(exports.replicateService, { klingT2vSubmit, klingI2vSubmit });
// ============ Queue-style API for Replicate Seedance ============
async function seedanceT2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    const replicate = ensureReplicate();
    ...
    try {
        ...
        if (e?.statusCode === 404 || e?.response?.status === 404 || (e?.message && e.message.includes('404'))) {
            ...
            throw new errorHandler_1.ApiError(errorMessage, 404, e);
        }
        const errorMessage = e?.message || e?.response?.data?.detail || e?.response?.data?.message || 'Replicate API error';
        throw new errorHandler_1.ApiError(`Failed to submit Seedance T2V job: ${errorMessage}`, 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
async function seedanceI2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    if (!body?.image)
        throw new errorHandler_1.ApiError('image is required', 400);
    ...
    try {
        ...
        const errorMessage = e?.message || e?.response?.data?.detail || e?.response?.data?.message || 'Replicate API error';
        throw new errorHandler_1.ApiError(`Failed to submit Seedance I2V job: ${errorMessage}`, 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
Object.assign(exports.replicateService, { seedanceT2vSubmit, seedanceI2vSubmit });
// ============ Queue-style API for Replicate PixVerse v5 ============
async function pixverseT2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    const replicate = ensureReplicate();
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit PixVerse T2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
async function pixverseI2vSubmit(uid, body) {
    if (!body?.prompt)
        throw new errorHandler_1.ApiError('prompt is required', 400);
    if (!body?.image)
        throw new errorHandler_1.ApiError('image is required', 400);
    ...
    try {
        ...
        await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { status: 'failed', error: e?.message || 'Replicate submit failed' });
        throw new errorHandler_1.ApiError('Failed to submit PixVerse I2V job', 502, e);
    }
    await generationHistoryRepository_1.generationHistoryRepository.update(uid, historyId, { provider: 'replicate', providerTaskId: predictionId });
    return { requestId: predictionId, historyId, model: modelBase, status: 'submitted' };
}
Object.assign(exports.replicateService, { pixverseT2vSubmit, pixverseI2vSubmit });
