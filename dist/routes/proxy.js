...
try {
    sharp_1.default.cache({ memory: 32, files: 0 });
...
router.get('/thumb/:path(*)', async (req, res) => {
    try {
        ...
        if (contentType.startsWith('image/')) {
            ...
            try {
                const transformer = (0, sharp_1.default)().resize({ width, withoutEnlargement: true }).webp({ quality });
                response.body?.pipe(transformer).pipe(res);
                return;
            }
            catch (e) {
                const placeholder = await (0, sharp_1.default)({ create: { width, height: Math.max(1, Math.round(width * 9 / 16)), channels: 3, background: { r: 12, g: 12, b: 14 } } }).webp({ quality }).toBuffer();
        ...
        if (contentType.startsWith('video/')) {
            ...
            if (contentLen && contentLen > 40 * 1024 * 1024) {
                const fallback = await (0, sharp_1.default)({ create: { width: Math.max(16, Math.min(width, 64)), height: Math.max(9, Math.min(Math.round(width * 9 / 16), 64)), channels: 3, background: { r: 12, g: 12, b: 14 } } }).webp({ quality: Math.min(quality, 50) }).toBuffer();
                res.status(200);
                res.setHeader('Content-Type', 'image/webp');
            ...
            try {
                ...
                if (ffmpegActive >= FFMPEG_MAX_CONCURRENCY) {
                    const busy = await (0, sharp_1.default)({ create: { width: Math.max(16, Math.min(width, 64)), height: Math.max(9, Math.min(Math.round(width * 9 / 16), 64)), channels: 3, background: { r: 12, g: 12, b: 14 } } }).webp({ quality: Math.min(quality, 50) }).toBuffer();
                    res.status(200);
                    res.setHeader('Content-Type', 'image/webp');
                ...
                const ws = (0, fs_1.createWriteStream)(inPath, { flags: 'w' });
                ...
                const execOk = await new Promise((resolve) => {
                    const child = (0, child_process_1.execFile)(String(ffmpeg_static_1.default), ['-y', '-ss', '0.5', '-i', inPath, '-frames:v', '1', '-vf', `scale=${width}:-1:force_original_aspect_ratio=decrease`, outPath], { timeout: 3500 }, (err) => {
                        if (err)
                ...
                if (execOk) {
                    ...
                    webp = await (0, sharp_1.default)(frame).webp({ quality }).toBuffer();
                }
                else {
                    // Fallback: tiny dark placeholder to avoid broken posters under load/timeouts
                    webp = await (0, sharp_1.default)({ create: { width: Math.max(16, Math.min(width, 64)), height: Math.max(9, Math.min(Math.round(width * 9 / 16), 64)), channels: 3, background: { r: 12, g: 12, b: 14 } } }).webp({ quality: Math.min(quality, 50) }).toBuffer();
                }
