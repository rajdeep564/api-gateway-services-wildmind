...
async function upsertFromHistory(uid, historyId, historyDoc, createdBy) {
    try {
        ...
        console.log('[Mirror][Upsert]', { historyId, uid, isPublic: historyDoc?.isPublic, visibility: historyDoc?.visibility, generationType: historyDoc?.generationType });
    }
    catch { }
...
async function updateFromHistory(uid, historyId, updates) {
    try {
        // eslint-disable-next-line no-console
        console.log('[Mirror][Update]', { historyId, uid, isPublic: updates?.isPublic, visibility: updates?.visibility, generationType: updates?.generationType });
    }
    catch { }
    const ref = firebaseAdmin_1.adminDb.collection('generations').doc(historyId);
    await ref.set({
