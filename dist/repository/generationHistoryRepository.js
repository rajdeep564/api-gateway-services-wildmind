...
exports.findBySoraVideoId = findBySoraVideoId;
...
async function list(uid, params) {
    ...
    if (Array.isArray(params.generationType)) {
        ...
        items = items.filter((it) => String(it.generationType) === want);
    }
    if (typeof params.dateStart === 'string' && typeof params.dateEnd === 'string') {
        ...
        items = items.filter((it) => {
            try {
                const created = new Date(it.createdAt || it.updatedAt);
    ...
    if (params.search && params.search.trim().length > 0) {
        ...
        items = items.filter((it) => String(it.prompt || '').toLowerCase().includes(needle));
    }
    items.sort((a, b) => {
        const av = a[sortBy];
        const bv = b[sortBy];
...
async function findBySoraVideoId(uid, soraVideoId) {
    const col = firebaseAdmin_1.adminDb.collection('generationHistory').doc(uid).collection('items');
    const snap = await col
