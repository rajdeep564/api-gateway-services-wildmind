...
async function generate(req, res, next) {
    try {
        ...
        const result = await falService_1.falService.generate(uid, { num_images, prompt, userPrompt, model, n, frameSize, style, uploadedImages, output_format, generationType, tags, nsfw, visibility, isPublic, aspect_ratio, resolution, seed, negative_prompt, characterName });
        let debitOutcome;
        try {
...
exports.falController = {
    generate,
    async briaExpandImage(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.briaExpandImage(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.bria.expand', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Expanded', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
    },
    async outpaintImage(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.outpaintImage(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.outpaint.image', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Outpainted', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
    },
    async topazUpscaleImage(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.topazUpscaleImage(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.topaz.upscale.image', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Upscaled', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
    },
    async seedvrUpscale(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.seedvrUpscale(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.seedvr.upscale', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
    ...
    async image2svg(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.image2svg(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.image2svg', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Converted to SVG', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
    },
    async recraftVectorize(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.recraftVectorize(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.recraft.vectorize', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Vectorized to SVG', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
    },
    async briaGenfill(req, res, next) {
        try {
            const uid = req.uid;
            const ctx = req.context || {};
            const result = await falService_1.falService.briaGenfill(uid, req.body || {});
            let debitOutcome;
            try {
                const requestId = result.historyId || ctx.idempotencyKey;
                if (requestId && typeof ctx.creditCost === 'number') {
                    debitOutcome = await creditsRepository_1.creditsRepository.writeDebitIfAbsent(uid, requestId, ctx.creditCost, ctx.reason || 'fal.bria.genfill', {
                        ...(ctx.meta || {}),
                        historyId: result.historyId,
                        provider: 'fal',
                        pricingVersion: ctx.pricingVersion,
            ...
            res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Generated', { ...result, debitedCredits: ctx.creditCost, debitStatus: debitOutcome }));
        }
        catch (err) {
            next(err);
        }
