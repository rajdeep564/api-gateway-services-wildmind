...
exports.sessionCacheStatus = sessionCacheStatus;
...
async function createSession(req, res, next) {
    try {
        ...
        try {
            const payload = (0, sessionStore_1.decodeJwtPayload)(sessionCookie) || {};
            const exp = typeof payload?.exp === 'number' ? payload.exp : undefined;
            const uid = user?.uid;
            if (uid) {
                await (0, sessionStore_1.cacheSession)(sessionCookie, { uid, exp, issuedAt: payload?.iat });
...
async function logout(req, res, next) {
    try {
        ...
        try {
            ...
            if (token) {
                await (0, sessionStore_1.deleteCachedSession)(token);
                const { env } = await Promise.resolve().then(() => __importStar(require('../../config/env')));
                if (env.redisDebug) {
                    // eslint-disable-next-line no-console
...
async function setSessionCookie(req, res, idToken) {
...
async function loginWithEmailPassword(req, res, next) {
    try {
        ...
        try {
            ...
            console.warn('[CONTROLLER][loginEmail] session cookie create failed', e?.message);
        }
    ...
    try {
        ...
        if (result.needsUsername) {
            ...
            try {
                ...
                console.warn('[CONTROLLER][googleSignIn:needsUsername] session cookie create failed', cookieErr?.message);
            }
            ...
            try {
                ...
                console.warn('[CONTROLLER][googleSignIn:existing] session cookie create failed', cookieErr?.message);
            }
...
async function sessionCacheStatus(req, res, _next) {
    try {
        const enabled = (0, redisClient_1.isRedisEnabled)();
        const token = req.cookies?.['app_session'];
        if (!enabled) {
            return res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Redis disabled', { enabled: false }));
        }
        if (!token) {
            return res.json((0, formatApiResponse_1.formatApiResponse)('success', 'No session cookie', { enabled: true, cached: false }));
        }
        const cached = await (0, sessionStore_1.getCachedSession)(token);
        return res.json((0, formatApiResponse_1.formatApiResponse)('success', 'OK', {
            enabled: true,
            cached: Boolean(cached?.uid),
            uid: cached?.uid || undefined,
        }));
    }
    catch (_e) {
        return res.json((0, formatApiResponse_1.formatApiResponse)('success', 'Error checking cache', { enabled: false }));
    }
}
