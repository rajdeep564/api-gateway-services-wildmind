...
console.log(
  "[AUTH][authController] Module loaded at",
...
async function createSession(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!idToken) {
      console.error(
        "[AUTH][createSession] ERROR: No idToken provided in request body",
      );
      console.log(
        "[AUTH][createSession] Request body keys:",
    ...
    try {
      ...
      console.error(
        "[AUTH][createSession] ERROR in authService.createSession:",
        {
    ...
    try {
      ...
      console.log(
        "[AUTH][createSession] setSessionCookie completed successfully, cookie length:",
      ...
      try {
        ...
        if (env.revokeFirebaseTokens) {
          ...
          try {
            ...
            console.log(
              "[AUTH][createSession] Revoked Firebase refresh tokens for user",
              { uid: user.uid },
            );
          } catch (revokeError) {
            console.warn(
              "[AUTH][createSession] Failed to revoke refresh tokens (non-fatal):",
              revokeError,
            );
          }
        } else {
          console.log(
            "[AUTH][createSession] Skipping Firebase refresh token revocation (disabled via env). REVOKE_FIREBASE_TOKENS=",
            env.revokeFirebaseTokens,
          );
        }

        console.log("[AUTH][createSession] Invalidated old sessions for user", {
          uid: user.uid,
        });
      } catch (error) {
        console.warn(
          "[AUTH][createSession] Failed to invalidate old sessions (non-fatal):",
          error,
        );
      }
    }
...
async function getCurrentUser(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    console.log("[AuthController]/me success", {
      uid,
      username: user?.username,
      plan: (user as any)?.planCode,
    });
  } catch (error) {
    console.error(
      "[AuthController]/me error",
      { uid: req.uid, cookies: Object.keys(req.cookies || {}) },
      error,
    );
    next(error);
  }
...
async function logout(req: Request, res: Response, next: NextFunction) {
  try {
    // SECURITY FIX: Remove from Redis cache BEFORE clearing the cookie.
    // If this step fails and a stale cache entry persists, a future user on the same
    // device could be served the wrong UID. Log as ERROR (not warn) so ops catch it.
    const token = (req.cookies as any)?.["app_session"];
    try {
      if (token) {
        await deleteCachedSession(token);
        console.log("[AUTH][logout] Redis session deleted successfully", {
          tokenPrefix: token.substring(0, 20) + "...",
        });
      }
    } catch (e) {
      // Log as ERROR — a stale Redis entry here is a direct identity mixup risk
      console.error(
        "[AUTH][logout] ⚠️ FAILED to delete Redis session — stale cache entry may persist!",
        {
          error: e,
          tokenPrefix: token ? token.substring(0, 20) + "..." : "N/A",
          risk: "A future request with this token could return the wrong user UID from cache.",
        },
      );
      // Non-fatal - continue with cookie clearing so user is logged out on client
    }
...
async function startEmailOtp(req: Request, res: Response, next: NextFunction) {
  ...
  async function verifyEmailOtp(req: Request, res: Response, next: NextFunction) {
    ...
    if (result.user?.uid) {
      ...
      try {
        ...
        if (existingUser && password) {
          ...
          console.log(
            "[AUTH][verifyEmailOtp] Invalidated old sessions (password reset scenario)",
            { uid: result.user.uid },
          );
        }
      } catch (error) {
        console.warn(
          "[AUTH][verifyEmailOtp] Failed to check/invalidate sessions (non-fatal):",
          error,
        );
      }
    }
    ...
    try {
      console.log("[CREDITS][verifyEmailOtp] Init start", {
      ...
      console.error("[CREDITS][verifyEmailOtp] Init error", {
        uid: (result.user as any)?.uid,
        err: e?.message,
      });
    }
...
async function setSessionCookie(req: Request, res: Response, idToken: string) {
  ...
  console.log(
    "[AUTH][setSessionCookie] Verifying ID token with Firebase Admin...",
  );
  try {
    ...
    console.log("[AUTH][setSessionCookie] ID token verified successfully:", {
      ...
      iatDate: decodedToken?.iat
        ? new Date(decodedToken.iat * 1000).toISOString()
        : "N/A",
      auth_time: decodedToken?.auth_time,
      auth_timeDate: decodedToken?.auth_time
    ...
    throw new ApiError(
      `Invalid ID token: ${verifyError?.message || "Token verification failed"}`,
      401,
    );
  }
  ...
  console.log(
    "[AUTH][setSessionCookie] Session cookie expiration (FIXED 14 DAYS):",
    {
      expiresIn,
  ...
  if (idTokenExpiresIn < minIdTokenTimeRemaining) {
    console.error(
      "[AUTH][setSessionCookie] ERROR: ID token expires too soon to create session cookie!",
      {
        idTokenExpiresIn,
    ...
    throw new ApiError(
      "ID token expires too soon. Please refresh and try again.",
      401,
    );
  }
  ...
  if (idTokenExp < now) {
    console.error(
      "[AUTH][setSessionCookie] ERROR: ID token is already expired!",
      {
        idTokenExp,
        idTokenExpDate: new Date(idTokenExp).toISOString(),
        now,
  ...
  const isLocalhostHost =
    ...
    reqHost === "127.0.0.1" ||
    reqHost === "::1" ||
    reqHost === "::ffff:127.0.0.1";
  const shouldUseSecureCookie =
    isProd || isHttpsRequest || (!isLocalhostHost && (isMobile || isWebView));

  console.log(
    "[AUTH][setSessionCookie] Creating session cookie with FIXED 14-day expiration:",
    {
      isProd,
  ...
  try {
    console.log(
      "[AUTH][setSessionCookie] Calling admin.auth().createSessionCookie...",
      {
        expiresIn,
        expiresInMs: expiresIn,
        expiresInSeconds: Math.floor(expiresIn / 1000),
    ...
    try {
      ...
      if (decodedSessionCookie?.exp) {
        ...
        if (actualDays < requestedDays) {
          console.warn(
            "[AUTH][setSessionCookie] WARNING: Firebase limited session cookie expiration!",
            {
              requestedDays,
              actualDays,
      ...
      console.warn(
        "[AUTH][setSessionCookie] Could not decode session cookie for verification:",
        decodeError,
      );
    }

    console.log(
      "[AUTH][setSessionCookie] Session cookie created successfully",
      {
    ...
    if (
      createError?.code === "auth/id-token-expired" ||
      ...
      console.error(
        "[AUTH][setSessionCookie] TOKEN_EXPIRED in createSessionCookie!",
        {
          currentTime: new Date().toISOString(),
          currentTimestamp: Date.now(),
    ...
    throw new ApiError(
      `Failed to create session cookie: ${createError?.message || "Unknown error"}`,
      500,
    );
  }
  ...
  const prodDomainHost = env.productionDomain
    ? new URL(env.productionDomain).hostname
    : env.productionWwwDomain
      ? new URL(env.productionWwwDomain).hostname.replace(/^www\./, "")
      : undefined;
  const studioDomainHost = env.productionStudioDomain
    ? new URL(env.productionStudioDomain).hostname
    : undefined;
  const isProductionLike =
    isProd ||
    (origin &&
      prodDomainHost &&
      (origin.includes(prodDomainHost) ||
        (studioDomainHost && origin.includes(studioDomainHost)))) ||
    (host &&
      prodDomainHost &&
      (host.includes(prodDomainHost) ||
        (studioDomainHost && host.includes(studioDomainHost))));
  ...
  if (isProductionLike && cookieDomain) {
    ...
    if (isFromWww) {
      ...
      console.log(
        "[AUTH][setSessionCookie] User accessed from www.wildmindai.com - using www.wildmindai.com cookie domain",
      );
      ...
      shouldSetDomain = true;
      console.log(
        "[AUTH][setSessionCookie] Direct access to studio.wildmindai.com - using .wildmindai.com cookie domain",
      );
    } else {
    ...
    const domainMatches = !!(
      dom &&
      (host === dom.replace(/^\./, "") || host.endsWith(dom))
    );
    ...
    if (isProductionLike) {
      console.error(
        "[AUTH][setSessionCookie] ⚠️⚠️⚠️ CRITICAL: COOKIE_DOMAIN is NOT SET! ⚠️⚠️⚠️",
      );
      console.error(
        "[AUTH][setSessionCookie] Cookies will NOT be shared across subdomains (www.wildmindai.com <-> studio.wildmindai.com)",
      );
      console.error(
        "[AUTH][setSessionCookie] To fix: Set COOKIE_DOMAIN=.wildmindai.com in Render.com environment variables",
      );
      console.error(
        "[AUTH][setSessionCookie] Then restart the backend service and have users log in again",
      );
    }
  }
  ...
  console.log("[AUTH][setSessionCookie] Domain setting decision:", {
    cookieDomain:
      cookieDomain || "(NOT SET - COOKIES WILL NOT SHARE ACROSS SUBDOMAINS!)",
    ...
    referer,
    shouldSetDomain,
    willSetDomain: shouldSetDomain
      ? finalCookieDomain
      : "(NOT SETTING - COOKIES WON'T SHARE!)",
    warning: !cookieDomain
      ? '⚠️⚠️⚠️ COOKIE_DOMAIN env var is NOT SET! Set it to ".wildmindai.com" in Render.com ⚠️⚠️⚠️'
      : !shouldSetDomain
        ? "⚠️ Domain will NOT be set - cookies won't share across subdomains"
  ...
  const useSameSiteNone =
    isProductionLike && !isWebView && shouldUseSecureCookie;
  ...
  console.log(
    "[AUTH][setSessionCookie] Setting cookie",
...
async function loginWithEmailPassword(
  ...
  try {
    ...
    console.log(`[AUTH][loginWithEmailPassword] Login successful`, {
      ...
      passwordLoginIdTokenPrefix:
        result.passwordLoginIdToken?.substring(0, 30) || "N/A",
      hasCustomToken: !!result.customToken,
      customTokenLength: result.customToken?.length || 0,
    });

    try {
      console.log("[CREDITS][loginEmail] Init start", {
        uid: (result.user as any)?.uid,
      });
      const init = await creditsService.ensureUserInit(result.user.uid as any);
      console.log("[CREDITS][loginEmail] Init done", init);
    } catch (e: any) {
      console.error("[CREDITS][loginEmail] Init error", {
        uid: (result.user as any)?.uid,
        err: e?.message,
      });
    }
...
async function googleSignIn(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    console.log(`[AUTH][googleSignIn] Google sign-in result`, {
    ...
    if (result.needsUsername) {
      ...
      try {
        console.log("[CREDITS][googleSignIn:needsUsername] Init start", {
          uid: (result.user as any)?.uid,
        });
        const init = await creditsService.ensureUserInit(
          result.user.uid as any,
        );
        console.log("[CREDITS][googleSignIn:needsUsername] Init done", init);
      } catch (e: any) {
        console.error("[CREDITS][googleSignIn:needsUsername] Init error", {
          uid: (result.user as any)?.uid,
          err: e?.message,
        });
      }
      ...
      try {
        console.log("[CREDITS][googleSignIn:existing] Init start", {
          uid: (result.user as any)?.uid,
        });
        const init = await creditsService.ensureUserInit(
          result.user.uid as any,
        ...
        console.error("[CREDITS][googleSignIn:existing] Init error", {
          uid: (result.user as any)?.uid,
          err: e?.message,
        });
      }
...
async function setGoogleUsername(
  ...
  try {
    ...
    try {
      console.log("[CREDITS][setGoogleUsername] Init start", {
        uid: (result.user as any)?.uid,
      });
      const init = await creditsService.ensureUserInit(result.user.uid as any);
      console.log("[CREDITS][setGoogleUsername] Init done", init);
    } catch (e: any) {
      console.error("[CREDITS][setGoogleUsername] Init error", {
        uid: (result.user as any)?.uid,
        err: e?.message,
      });
    }
...
async function refreshSession(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!idToken) {
      throw new ApiError(
        "Missing idToken - client must provide fresh Firebase ID token",
        400,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[AUTH] Failed to cache refreshed session (non-fatal):",
        cacheError,
      );
    }
...
export async function sessionCacheStatus(
  req: Request,
  res: Response,
  _next: NextFunction,
) {
  try {
    ...
    if (!token) {
      return res.json(
        formatApiResponse("success", "No session cookie", {
          enabled: true,
          cached: false,
        }),
      );
    ...
    return res.json(
      formatApiResponse("success", "Error checking cache", { enabled: false }),
    );
  }
}
...
export async function debugSession(
  ...
  try {
    ...
    const appSessionCookie = allCookies.find((c) =>
    ...
    console.log("[AUTH][debugSession] Cookie debug info:", {
      ...
      cookieHeaderPreview:
        ...
        (cookieHeader.length > 150 ? "..." : ""),
      allCookies: allCookies,
      hasAppSessionInHeader,
      appSessionCookie: appSessionCookie
        ? appSessionCookie.length > 50
          ? appSessionCookie.substring(0, 50) + "..."
          : appSessionCookie
        : null,
      hostname: req.hostname,
      origin: req.headers.origin,
      ...
      try {
        ...
        try {
          ...
          const isIssuerMismatch =
            sessionError?.message?.includes("iss") ||
    ...
    try {
      ...
      if (cached) {
        ...
        cacheStatus = {
          ...
          exp: cached.exp,
          expDate: cached.exp
            ? new Date(cached.exp * 1000).toISOString()
            : null,
          issuedAt: cached.issuedAt,
          issuedAtDate: cached.issuedAt
            ? new Date(cached.issuedAt * 1000).toISOString()
            : null,
          isExpired,
          expiresIn: cached.exp ? Math.max(0, cached.exp - nowSec) : null,
          expiresInDays: cached.exp
            ? Math.floor((cached.exp - nowSec) / (24 * 60 * 60))
            : null,
        };
      } else {
    ...
    if (token && decoded) {
      ...
      if (decoded.exp) {
        ...
        jwtInfo.ageInDays = decoded.iat
          ? Math.floor((nowSec - decoded.iat) / (24 * 60 * 60))
          : null;
      }
    } else if (token) {
    ...
    return res.json(
      formatApiResponse("success", "Session debug info", {
        timestamp: new Date().toISOString(),
        hasToken,
        ...
        cookieHeaderAnalysis: {
          ...
          diagnosis: !hasAppSessionInHeader
            ? {
                ...
                explanation:
                  "The app_session cookie is not being sent with this request. This means the cookie either:",
                possibleCauses: [
                ...
                networkTabCheck:
                  "Open DevTools → Network tab → Find /api/auth/me request → Headers → Request Headers → Check if Cookie header includes app_session",
              }
        ...
        recommendations: !hasToken
          ...
          : verificationStatus === "verification_failed"
            ? [
                "Session token verification failed - user may be logged out",
                "Check backend logs for detailed error",
              ]
            : jwtInfo?.isExpired
              ? ["Session token is expired - user needs to log in again"]
              : jwtInfo?.expiresInDays !== undefined &&
                  jwtInfo.expiresInDays < 1
                ? ["Session expires soon - refresh may be needed"]
                : [
                    "Session appears valid",
                    `Expires in ${jwtInfo?.expiresInDays || "unknown"} days`,
                  ],
      }),
    );
  } catch (error: any) {
    return res.json(
      formatApiResponse("error", "Debug endpoint error", {
        error: error?.message,
        stack: error?.stack,
      }),
    );
