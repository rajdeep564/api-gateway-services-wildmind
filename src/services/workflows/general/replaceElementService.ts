...
export interface ReplaceElementRequest {
    imageUrl: string;
    from: string;
    to: string;
    frameSize?: string;
...
export const replaceElement = async (uid: string, req: ReplaceElementRequest) => {
    ...
    try {
        console.log('[replaceElement] Running model', { model: modelBase, input: inputPayload });
        ...
        console.error('[replaceElement] Error', e);
        await generationHistoryRepository.update(uid, historyId, {
            status: "failed",
