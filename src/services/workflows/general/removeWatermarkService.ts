...
const DEFAULT_PROMPT = `Remove the watermark from the reference image without affecting any other part of the image. Carefully reconstruct the underlying details so the area blends naturally with the surroundings. Preserve the original colors, lighting, sharpness, and overall image quality.`;

export const removeWatermark = async (uid: string, req: RemoveWatermarkRequest) => {
    ...
    const finalPrompt = req.prompt || DEFAULT_PROMPT;
    ...
    try {
        console.log('[removeWatermark] Running model', { model: modelBase, input: inputPayload });
