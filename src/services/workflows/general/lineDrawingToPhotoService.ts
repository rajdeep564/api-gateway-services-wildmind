...
export interface LineDrawingToPhotoRequest {
    imageUrl: string;
    prompt?: string;
    frameSize?: string;
...
const DEFAULT_PROMPT = `Convert the attached black-and-white line art into a fully photorealistic image.
...
export const lineDrawingToPhoto = async (uid: string, req: LineDrawingToPhotoRequest) => {
    ...
    const legacyId = await replicateRepository.createGenerationRecord(
        {
            prompt: req.prompt || "Line Drawing to Photo...",
            model: modelBase,
            isPublic: req.isPublic ?? true
        },
    ...
    try {
        console.log('[lineDrawingToPhoto] Running model', { model: modelBase, input: inputPayload });
        ...
        console.error('[lineDrawingToPhoto] Error', e);
        await generationHistoryRepository.update(uid, historyId, {
            status: "failed",
