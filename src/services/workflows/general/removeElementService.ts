...
export const removeElement = async (uid: string, req: RemoveElementRequest) => {
    ...
    // Hardcoded logic for prompt: Remove "user_prompt" from Image
    const finalPrompt = `Remove "${req.prompt}" from Image`;
    ...
    try {
        console.log('[removeElement] Running model', { model: modelBase, input: inputPayload });
        ...
        console.error('[removeElement] Error', e);
        await generationHistoryRepository.update(uid, historyId, {
            status: "failed",
