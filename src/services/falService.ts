...
function tryConvertProxyResourceToZataUrl(
  src: string,
): { key: string; publicUrl: string } | null {
  try {
    if (!src) return null;
...
async function generate(
  ...
  const hasDialogueInputs =
    Array.isArray((payload as any).inputs) &&
    (payload as any).inputs.length > 0;
  const hasTextInPayload =
    typeof (payload as any).text === "string" &&
    (payload as any).text.trim().length > 0;
  ...
  const canSkipPrompt =
    isAudioRequest && (hasDialogueInputs || hasTextInPayload);

  if (!canSkipPrompt && !prompt)
    throw new ApiError("Prompt is required (from falService)", 400);
  ...
  try {
    ...
    if (!validation.valid) {
      console.log("[falService.generate] Validation failed", {
        uid,
        model,
    ...
    console.warn(
      "[falService.generate] Validation check failed, continuing:",
      e?.message,
    );
  }
  ...
  const { historyId } = await generationHistoryRepository.create(uid, {
    ...
    ...(generationType === "text-to-character" && characterName
      ? { characterName }
      : {}),
  ...
  const inputFolder =
    generationType === "text-to-character" ? "character" : "input";
  let publicImageUrls: string[] = [];
  const isFlux2Pro =
    modelLower.includes("flux-2-pro") || modelLower.includes("flux 2 pro");
  ...
  try {
    ...
    for (const src of uploadedImages || []) {
      ...
      try {
        ...
        if (proxyAsZata) {
          stored = {
            publicUrl: proxyAsZata.publicUrl,
            key: proxyAsZata.key,
            originalUrl: src,
          };
          inputPersisted.push({
        ...
        if (isFlux2Pro) {
          ...
          if (/^data:/i.test(src)) {
            ...
            if (match) {
              ...
              console.warn(
                "[falService] Invalid data URI, skipping:",
                src.substring(0, 50),
              );
            ...
            try {
              ...
              if (resp.status < 200 || resp.status >= 300) {
                console.warn(
                  "[falService] Failed to download from public URL for Flux 2 Pro resize, using uploaded URL directly:",
                  uploaded.publicUrl.substring(0, 80),
                );
                ...
                inputPersisted.push({
                  ...
                  originalUrl: stored.originalUrl || src,
                });
                publicImageUrls.push(stored.publicUrl);
                continue;
              }
              ...
              console.warn(
                "[falService] Failed to process image for Flux 2 Pro resize, skipping:",
          ...
          if (isDataUri) {
            ...
            console.log(
              "[falService] 📤 Uploading data URI to Zata for Seedream 4.5:",
              {
            ...
            stored = await uploadDataUriToZata({
              dataUri: src,
              keyPrefix,
              fileName: `input-${++idx}`,
            });
            ...
            throw new ApiError(
              "Blob URLs are not supported. Please convert to data URI or public URL first.",
              400,
            );
          } else {
            // Regular URL - upload to Zata (handles Zata URLs via S3)
            stored = await uploadFromUrlToZata({
              sourceUrl: src,
              keyPrefix,
              fileName: `input-${++idx}`,
            });
    ...
    if ((isNanoBananaPro || isNanoBanana2) && payloadImageUrls.length > 0) {
      for (const src of payloadImageUrls) {
        ...
        const alreadyPersisted = inputPersisted.some(
          (img) => img.url === src || img.originalUrl === src,
        );
        ...
        try {
          ...
          if (stored && stored.publicUrl) {
            ...
            publicImageUrls.push(stored.publicUrl);
            console.log(
              "[falService] ✅ Successfully processed Nano Banana image_url:",
              {
          ...
          console.error(
            "[falService] ❌ Failed to process Nano Banana image_url:",
            {
  ...
  const legacyId = await falRepository.createGenerationRecord(
    {
      prompt,
      model,
      n: imagesRequested,
  ...
  if (
    ...
    const hasImages =
      Array.isArray(uploadedImages) && uploadedImages.length > 0;
    modelEndpoint = hasImages ? "fal-ai/flux-2-pro/edit" : "fal-ai/flux-2-pro";
    ...
    (modelLower.includes("seedream") &&
      (modelLower.includes("4.5") ||
        modelLower.includes("v4.5") ||
        modelLower.includes("v45")))
  ) {
  ...
  if (modelLower.includes("flux-pro")) {
    ...
    if (modelLower.includes("ultra"))
      modelEndpoint = "fal-ai/flux-pro/v1.1-ultra";
  ...
  const transformPromptWithImageReferences = (
    promptText: string,
    imageCount: number,
  ): string => {
  ...
  let finalPrompt =
    hasCharacterRefs && hasImages
      ? transformPromptWithImageReferences(
          prompt,
          publicImageUrls.length || uploadedImages.length,
        )
      : prompt;
  ...
  const wantsChatterbox =
    modelLowerRaw.includes("chatterbox") ||
    modelLowerRaw.includes("multilingual");
  const isChatterboxSts =
    (typeof (payload as any).source_audio_url === "string" &&
      (payload as any).source_audio_url.length > 0) ||
    modelLowerRaw.includes("speech-to-speech") ||
    modelLowerRaw.includes("chatterboxhd");
  ...
  const hasInputsArray =
    Array.isArray((payload as any).inputs) &&
    (payload as any).inputs.length > 0;
  const hasText =
    typeof (payload as any).text === "string" &&
    (payload as any).text.trim().length > 0;
  const isElevenDialogue =
    hasInputsArray ||
    modelLowerRaw.includes("dialogue") ||
    modelLowerRaw.includes("text-to-dialogue");
  const isElevenTts =
    hasText ||
    modelLowerRaw.includes("tts") ||
    modelLowerRaw.includes("text-to-speech") ||
    modelLowerRaw.includes("text-to-voice");
  const isElevenSfx =
    hasText &&
    (modelLowerRaw.includes("sfx") ||
      modelLowerRaw.includes("sound-effect") ||
      modelLowerRaw.includes("sound-effects") ||
      generationType === "sfx");
  const isMayaTts =
    hasText ||
    modelLowerRaw.includes("maya") ||
    modelLowerRaw.includes("maya-1") ||
    modelLowerRaw.includes("maya-1-voice");
  const isChatterboxMultilingual =
    hasText ||
    modelLowerRaw.includes("chatterbox") ||
    modelLowerRaw.includes("multilingual");
  ...
  if (wantsMaya && isMayaTts) {
    ...
    try {
      ...
      if ((payload as any).temperature != null)
        inputBody.temperature = (payload as any).temperature;
      if ((payload as any).top_p != null)
        inputBody.top_p = (payload as any).top_p;
      if ((payload as any).max_tokens != null)
        inputBody.max_tokens = (payload as any).max_tokens;
      if ((payload as any).repetition_penalty != null)
        inputBody.repetition_penalty = (payload as any).repetition_penalty;
      if ((payload as any).output_format)
        inputBody.output_format = (payload as any).output_format;

      console.log("[falService.generate] Calling Maya TTS model:", {
        mayaEndpoint,
        input: {
          ...inputBody,
          text:
            String(inputBody.text).slice(0, 120) +
            (String(inputBody.text).length > 120 ? "..." : ""),
        },
      });
      const result = await fal.subscribe(
        mayaEndpoint as any,
        { input: inputBody, logs: true } as unknown as any,
      );

      const audioUrl: string | undefined = (result as any)?.data?.audio?.url;
      if (!audioUrl)
        throw new ApiError("No audio URL returned from FAL Maya API", 502);
      ...
      const audioObj = {
        id: result.requestId || `fal-${Date.now()}`,
        url: stored.publicUrl,
        storagePath: stored.key,
      ...
      try {
        ...
        await updateMirror(uid, historyId, {
          status: "failed" as any,
          error: falError.message,
        });
      } catch (mirrorErr) {
        console.error(
          "[falService.generate][maya] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  if (wantsChatterbox && isChatterboxMultilingual) {
    ...
    try {
      ...
      if (
        ...
        const isZataUrl =
          voiceValue.includes("idr01.zata.ai") ||
          voiceValue.includes("zata.ai");

        if (!isZataUrl) {
          // This is a custom voice URL from external source - upload it to Zata storage
          const username = creator?.username || uid;
          const voiceFileName =
            (payload as any).voice_file_name || `custom-voice-${Date.now()}`;
          const keyPrefix = `users/${username}/inputaudio`;

          try {
            console.log(
              "[falService.generate] Uploading custom voice file to Zata:",
              { originalUrl: voiceValue, fileName: voiceFileName },
            );
            ...
            console.log(
              "[falService.generate] Custom voice file uploaded successfully:",
              { storedUrl: voiceValue, storagePath: voiceStored.key },
            );
          } catch (uploadErr: any) {
            console.error(
              "[falService.generate] Failed to upload custom voice file to Zata, using original URL:",
          ...
          console.log(
            "[falService.generate] Voice URL is already a Zata URL, using directly:",
            voiceValue,
          );
        }
      }
      ...
      if ((payload as any).custom_audio_language)
        inputBody.custom_audio_language = (
          payload as any
        ).custom_audio_language;
      if ((payload as any).exaggeration != null)
        inputBody.exaggeration = (payload as any).exaggeration;
      if ((payload as any).temperature != null)
        inputBody.temperature = (payload as any).temperature;
      if ((payload as any).cfg_scale != null)
        inputBody.cfg_scale = (payload as any).cfg_scale;
      if ((payload as any).seed != null) inputBody.seed = (payload as any).seed;
      if ((payload as any).audio_url)
        inputBody.audio_url = (payload as any).audio_url;

      console.log(
        "[falService.generate] Calling Chatterbox multilingual TTS model:",
        {
          chatterEndpoint,
          input: {
            ...inputBody,
      ...
      if (!audioUrl)
        throw new ApiError(
          "No audio URL returned from FAL Chatterbox API",
          502,
        );
      ...
      try {
        stored = await uploadFromUrlToZata({
          sourceUrl: audioUrl,
          keyPrefix: `users/${username}/audio/${historyId}`,
          fileName: "chatterbox-multilingual",
        });
      } catch (e) {
        stored = { publicUrl: audioUrl, key: "" };
      }
      ...
      try {
        ...
        console.error(
          "[falService.generate][chatterbox] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  if (isChatterboxSts) {
    ...
    try {
      ...
      if ((payload as any).target_voice)
        inputBody.target_voice = (payload as any).target_voice;
      if ((payload as any).target_voice_audio_url)
        inputBody.target_voice_audio_url = (
          payload as any
        ).target_voice_audio_url;
      if ((payload as any).high_quality_audio != null)
        inputBody.high_quality_audio = (payload as any).high_quality_audio;

      console.log("[falService.generate] Calling Chatterbox STS model:", {
        stsEndpoint,
        input: { ...inputBody },
      });
      const result = await fal.subscribe(
      ...
      if (!audioUrl)
        throw new ApiError(
          "No audio URL returned from FAL Chatterbox STS API",
          502,
        );
      ...
      try {
        ...
        console.error(
          "[falService.generate][chatterbox-sts] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  if (wantsEleven && (isElevenDialogue || isElevenTts || isElevenSfx)) {
    ...
    if (isElevenSfx) {
      ...
      try {
        ...
        if ((payload as any).duration_seconds != null)
          inputBody.duration_seconds = Number(
            (payload as any).duration_seconds,
          );
        if ((payload as any).prompt_influence != null)
          inputBody.prompt_influence = Number(
            (payload as any).prompt_influence,
          );
        if ((payload as any).output_format)
          inputBody.output_format = (payload as any).output_format;
        if ((payload as any).loop != null)
          inputBody.loop = Boolean((payload as any).loop);

        console.log("[falService.generate] Calling ElevenLabs SFX model:", {
          sfxEndpoint,
          input: {
            ...inputBody,
            text:
        ...
        if (!audioUrl)
          throw new ApiError(
            "No audio URL returned from FAL ElevenLabs SFX API",
            502,
          );
        ...
        await generationHistoryRepository.update(uid, historyId, {
          ...
          ...((payload as any).fileName
            ? { fileName: (payload as any).fileName }
            : {}),
          ...((payload as any).lyrics
            ? { lyrics: (payload as any).lyrics }
            : {}),
        } as any);
        await falRepository.updateGenerationRecord(legacyId, {
          status: "completed",
        ...
        const message =
          err?.message ||
          "Failed to generate audio with FAL ElevenLabs SFX API";
        try {
          await falRepository.updateGenerationRecord(legacyId, {
            status: "failed",
            error: message,
          ...
          console.error(
            "[falService.generate][eleven-sfx] Failed to mirror error state:",
            mirrorErr,
          );
        }
    ...
    if (isElevenTts) {
      ...
      try {
        ...
        if ((payload as any).exaggeration != null)
          inputBody.exaggeration = Number((payload as any).exaggeration);
        if ((payload as any).temperature != null)
          inputBody.temperature = Number((payload as any).temperature);
        if ((payload as any).cfg_scale != null)
          inputBody.cfg_scale = Number((payload as any).cfg_scale);
        if ((payload as any).stability != null)
          inputBody.stability = Number((payload as any).stability);
        if ((payload as any).similarity_boost != null)
          inputBody.similarity_boost = Number(
            (payload as any).similarity_boost,
          );
        if ((payload as any).style != null)
          inputBody.style = Number((payload as any).style);
        if ((payload as any).speed != null)
          inputBody.speed = Number((payload as any).speed);

        console.log("[falService.generate] Calling ElevenLabs TTS model:", {
          ttsEndpoint,
          input: {
            ...inputBody,
            text:
        ...
        if (!audioUrl)
          throw new ApiError(
            "No audio URL returned from FAL ElevenLabs TTS API",
            502,
          );
        ...
        const falError = buildFalApiError(err, {
          fallbackMessage:
            "Failed to generate audio with FAL ElevenLabs TTS API",
        ...
        try {
          ...
          console.error(
            "[falService.generate][eleven-tts] Failed to mirror error state:",
            mirrorErr,
          );
        }
        throw falError;
    ...
    if (isElevenDialogue) {
      ...
      try {
        const inputs = (payload as any).inputs || [
          { text: finalPrompt, voice: (payload as any).voice || "Rachel" },
        ];
        const inputBody: any = { inputs };
        if ((payload as any).stability != null)
          inputBody.stability = (payload as any).stability;
        if ((payload as any).use_speaker_boost != null)
          inputBody.use_speaker_boost = (payload as any).use_speaker_boost;
        if ((payload as any).pronunciation_dictionary_locators)
          inputBody.pronunciation_dictionary_locators = (
            payload as any
          ).pronunciation_dictionary_locators;
        if ((payload as any).seed != null)
          inputBody.seed = (payload as any).seed;

        console.log(
          "[falService.generate] Calling ElevenLabs dialogue model:",
          {
            dialogueEndpoint,
            input: { ...inputBody, inputs: `[${(inputs || []).length} items]` },
          },
        );
        const result = await fal.subscribe(
          dialogueEndpoint as any,
          { input: inputBody, logs: true } as unknown as any,
        );
        ...
        if (!audioUrl)
          throw new ApiError(
            "No audio URL returned from FAL ElevenLabs API",
            502,
          );
        ...
        try {
          ...
          console.error(
            "[falService.generate][eleven-dialogue] Failed to mirror error state:",
            mirrorErr,
          );
        }
        throw falError;
  ...
  if (modelLower.includes("flux-pro")) {
    ...
    if ((payload as any).image_size) {
      ...
      console.log(
        "[falService] ✅ Using custom image_size for Flux Pro:",
        JSON.stringify(inputBody.image_size),
      );
    } else {
    ...
    try {
      console.log("[falService] Submitting Flux Pro request to FAL:", {
        modelEndpoint,
        input: inputBody,
      });
      const result = await fal.subscribe(modelEndpoint as any, {
        ...
        onQueueUpdate: (update) => {
          if (update.status === "IN_PROGRESS") {
            update.logs
              .map((log) => log.message)
              .forEach((msg) => console.log(`[FAL Flux Log] ${msg}`));
          }
        },
      });
      ...
      if (
        ...
        console.error(
          "[falService] No images returned from Flux Pro generation:",
          result,
        );
        throw new ApiError(
          "No images returned from Flux Pro generation",
          502,
          result,
        );
      }
      ...
      const storedImages = await Promise.all(
        images.map(async (img: any, idx: number) => {
          ...
          try {
            ...
            console.warn(
              "[falService] Failed to upload Flux Pro image to Zata, using original:",
              e,
            );
            return {
              id: `${requestId}-${idx}`,
      ...
      markGenerationCompleted(uid, historyId, {
        ...
        console.error("[falService] Optimization failed:", err),
      );
  ...
  if (isNanoBananaPro) {
    console.log(
      `[falService] 🚀 Handling Nano Banana Pro request: ${modelEndpoint}`,
    );
    ...
    if (!validatedPrompt) {
      throw new ApiError(
        "Prompt is required for Google Nano Banana Pro image generation",
        400,
      );
    }
    ...
    if ((payload as any).limit_generations !== undefined)
      inputBody.limit_generations = (payload as any).limit_generations;
    if ((payload as any).enable_web_search !== undefined)
      inputBody.enable_web_search = (payload as any).enable_web_search;
    if ((payload as any).seed != null)
    ...
    try {
      console.log("[falService] Submitting Nano Banana Pro request to FAL:", {
        modelEndpoint,
        input: inputBody,
      });
      const result = await fal.subscribe(modelEndpoint as any, {
      ...
      if (finalImages.length === 0) {
        console.error(
          "[falService] No images returned from Nano Banana Pro generation:",
          result,
        );
        throw new ApiError(
          "No images returned from Nano Banana Pro generation",
          502,
          result,
        );
      }
      ...
      // Upload images to Zata
      const storedImages = await Promise.all(
        finalImages.map(async (img: any, idx: number) => {
          ...
          try {
            ...
            console.warn(
              "[falService] Failed to upload Nano Banana image to Zata:",
              e,
            );
            return {
              id: `${requestId}-${idx}`,
  ...
  try {
    const fileNameForIndex = (index: number) =>
      buildGenerationImageFileName(historyId, index);
    const imagePromises = Array.from(
      { length: imagesRequested },
      async (_, index) => {
        const input: any = {
          prompt: finalPrompt,
          output_format,
        ...
        if (modelEndpoint.includes("flux-2-pro")) {
          ...
          if (explicit) {
            ...
            const aspectMap: Record<
              string,
              {
                enum: string;
                enum1K?: string;
                custom2K?: { width: number; height: number };
              }
            > = {
              "1:1": {
                enum: "square_hd",
                enum1K: "square_hd",
                custom2K: { width: 2048, height: 2048 },
              }, // 1K: square_hd (1024x1024), 2K: 2048x2048
              "4:3": {
                enum: "landscape_4_3",
                custom2K: { width: 2048, height: 1536 },
              },
              "3:4": {
                enum: "portrait_4_3",
                custom2K: { width: 1536, height: 2048 },
              },
              "16:9": {
                enum: "landscape_16_9",
                custom2K: { width: 2048, height: 1152 },
              },
              "9:16": {
                enum: "portrait_16_9",
                custom2K: { width: 1152, height: 2048 },
              },
              square_hd: {
                enum: "square_hd",
                enum1K: "square_hd",
            ...
            const mapping =
              aspectMap[String(resolvedAspect)] || aspectMap["1:1"];
          ...
          if ((payload as any).safety_tolerance)
            input.safety_tolerance = String((payload as any).safety_tolerance);
          if ((payload as any).enable_safety_checker !== undefined)
            input.enable_safety_checker = Boolean(
              (payload as any).enable_safety_checker,
            );
          if ((payload as any).seed != null)
          ...
          if (isEditEndpoint) {
            const refs =
              publicImageUrls.length > 0 ? publicImageUrls : uploadedImages;
          ...
          if (isSeedream45) {
            ...
            if (explicitSize) {
              ...
              console.log("[falService] Seedream 4.5 resolution check:", {
                resolution,
                hasResolution: !!resolution,
                payloadResolution: (payload as any).resolution,
              });
              if (resolution === "4K") {
                input.image_size = "auto_4K";
                console.log(
                  "[falService] ✅ Seedream 4.5: Using auto_4K for 4K resolution",
                );
              } else if (resolution === "2K" || resolution === "1K") {
                input.image_size = "auto_2K";
                console.log(
                  "[falService] ✅ Seedream 4.5: Using auto_2K for",
                  resolution,
                  "resolution",
                );
              } else {
                ...
                console.log(
                  "[falService] ⚠️ Seedream 4.5: No resolution provided, using aspect ratio enum:",
                  mappedEnum,
                );
              }
            }
            ...
            if ((payload as any).max_images != null) {
              ...
              if (Number.isFinite(maxImg))
                input.max_images = Math.max(
                  1,
                  Math.min(15, Math.round(maxImg)),
                );
            }
            if ((payload as any).seed != null)
              input.seed = Number((payload as any).seed);
            if ((payload as any).sync_mode != null)
            ...
            if (isEditEndpoint) {
              ...
              if (publicImageUrls.length === 0 && uploadedImages.length > 0) {
                const hasDataUris = uploadedImages.some(
                  (url: string) =>
                    /^data:/i.test(url) || url.startsWith("blob:"),
                );
                if (hasDataUris) {
                  throw new ApiError(
              ...
              if (Array.isArray(refs) && refs.length > 0) {
                ...
                const invalidUrls = refs.filter(
                  (url: string) =>
                    /^data:/i.test(url) || url.startsWith("blob:"),
                );
                if (invalidUrls.length > 0) {
                  console.error(
                    "[falService] ❌ ERROR: Attempting to send data URIs or blob URLs to Fal:",
                    {
          ...
          if ((payload as any).enable_web_search !== undefined) {
            input.enable_web_search = Boolean(
              (payload as any).enable_web_search,
            );
          }
        ...
        if (modelEndpoint.startsWith("fal-ai/imagen4/")) {
          if ((payload as any).resolution)
            input.resolution = (payload as any).resolution; // '1K' | '2K'
          if ((payload as any).seed != null) input.seed = (payload as any).seed;
          if ((payload as any).negative_prompt)
            input.negative_prompt = (payload as any).negative_prompt;
        }
        if (
          modelEndpoint.endsWith("/edit") &&
          !modelEndpoint.includes("nano-banana-pro")
        ) {
          ...
          if (prompt && refs.length > 0) {
            ...
            try {
              console.log(
                "[falService.generate] Character reference mapping:",
                {
        ...
        try {
          console.log("[falService.generate] request", {
            modelEndpoint,
            input,
          });
        } catch {}
        ...
        const isEditEndpoint =
          (input as any)._isEditEndpoint === true ||
          modelEndpoint.includes("/edit");
        const isSeedream45I2I =
          isSeedream45 && isEditEndpoint && publicImageUrls.length > 0;

        let result: any;
        if (isSeedream45I2I) {
          // Use queue mode for Seedream 4.5 image-to-image to avoid timeout
          console.log(
            "[falService] Using queue mode for Seedream 4.5 image-to-image to avoid timeout",
          );
          const { request_id } = await fal.queue.submit(
            modelEndpoint as any,
            { input } as any,
          );
          ...
          while (attempts < maxAttempts) {
            ...
            try {
              const status: any = await fal.queue.status(
                modelEndpoint as any,
                { requestId: request_id, logs: true } as any,
              );
              ...
              if (statusValue === "COMPLETED" || statusValue === "completed") {
                console.log(
                  "[falService] ✅ Queue task completed, extracting result...",
                );
                ...
                if (
                  status?.data?.images &&
                  Array.isArray(status.data.images) &&
                  status.data.images.length > 0
                ) {
                  console.log(
                    "[falService] ✅ Found images in status.data.images",
                  );
                ...
                // 2. Check status.data.image.url (single image)
                  ...
                  console.log(
                    "[falService] ✅ Found image in status.data.image.url",
                  );
                  queueResult = {
                    data: { images: [{ url: status.data.image.url }] },
                  };
                  foundResult = true;
                }
                // 3. Check status.data.image_url (direct URL)
                  ...
                  queueResult = {
                    data: { images: [{ url: status.data.image_url }] },
                  };
                  foundResult = true;
                }
                ...
                // 5. Check status.output.images
                  ...
                  Array.isArray(status.output.images) &&
                  status.output.images.length > 0
                ) {
                  console.log(
                    "[falService] ✅ Found images in status.output.images",
                  );
                ...
                // 6. Check status.output.image.url
                  ...
                  console.log(
                    "[falService] ✅ Found image in status.output.image.url",
                  );
                  queueResult = {
                    data: { images: [{ url: status.output.image.url }] },
                  };
                  foundResult = true;
                }
                // 7. Check if status.data itself is the result (sometimes the whole data is the result)
                  ...
                  if (
                    ...
                    (dataStr.includes(".jpg") ||
                      dataStr.includes(".png") ||
                      dataStr.includes(".jpeg"))
                  ) {
                    console.log(
                      "[falService] ✅ Found URL-like data in status.data, attempting extraction",
                    );
                    ...
                    if (foundUrl) {
                      console.log(
                        "[falService] ✅ Extracted URL from status.data:",
                        foundUrl.substring(0, 80),
                      );
                ...
                if (!foundResult && status?.response_url) {
                  try {
                    console.log(
                      "[falService] Result not in status response, fetching from response_url:",
                      status.response_url.substring(0, 80),
                    );
                    const falKey = env.falKey as string;
                    ...
                    if (!responseUrlFetch.ok) {
                      const errorText = await responseUrlFetch
                        .text()
                        .catch(() => responseUrlFetch.statusText);
                      console.warn(
                        "[falService] Failed to fetch from response_url:",
                        {
                      ...
                      const queueResultData: any =
                        await responseUrlFetch.json();
                      console.log(
                        "[falService] ✅ Raw response from response_url:",
                        {
                          status: responseUrlFetch.status,
                          contentType:
                            responseUrlFetch.headers.get("content-type"),
                          dataKeys: queueResultData
                            ? Object.keys(queueResultData)
                            : [],
                      ...
                      console.log(
                        "[falService] ✅ Result fetched from response_url:",
                        {
                          hasData: !!queueResultData?.data,
                          hasImages: !!(
                            queueResultData?.data &&
                            "images" in queueResultData.data &&
                            Array.isArray(queueResultData.data.images)
                          ),
                          hasImage: !!(
                            queueResultData?.data &&
                            "image" in queueResultData.data
                          ),
                          resultKeys: Object.keys(queueResultData || {}),
                          dataKeys: queueResultData?.data
                            ? Object.keys(queueResultData.data)
                            : [],
                      ...
                      if (
                        ...
                        console.log(
                          "[falService] ✅ Found images in queueResult.data.images",
                        );
                        ...
                        console.log(
                          "[falService] ✅ Found image in queueResult.data.image.url",
                        );
                        queueResult = {
                          data: { images: [{ url: data.image.url }] },
                        };
                        foundResult = true;
                        break;
                      } else if (
                        "images" in queueResultData &&
                        Array.isArray((queueResultData as any).images) &&
                        (queueResultData as any).images.length > 0
                      ) {
                        console.log(
                          "[falService] ✅ Found images at top level of queueResult",
                        );
                        queueResult = {
                          data: { images: (queueResultData as any).images },
                        };
                        foundResult = true;
                        break;
                      } else if (
                        ...
                        console.log(
                          "[falService] ✅ Found image_url in queueResult.data",
                        );
                        queueResult = {
                        ...
                        console.warn(
                          "[falService] ⚠️ Result fetched from response_url but no images found, structure:",
                          {
                            resultKeys: Object.keys(queueResultData || {}),
                    ...
                    console.warn(
                      "[falService] Failed to fetch from response_url:",
                      {
                        ...
                        status:
                          responseUrlError?.status ||
                          responseUrlError?.statusCode,
                ...
                if (!foundResult) {
                  try {
                    console.log(
                      "[falService] Trying to fetch from queue URL directly...",
                    );
                    ...
                    if (status?.response_url) {
                      ...
                      const urlMatch = status.response_url.match(
                        /https:\/\/queue\.fal\.run\/([^\/]+\/[^\/]+)\//,
                      );
                      if (urlMatch && urlMatch[1]) {
                        queueModelPath = urlMatch[1];
                        console.log(
                          "[falService] Extracted model path from response_url:",
                          queueModelPath,
                        );
                      }
                    }
                    ...
                    if (!queueResponse.ok) {
                      const text = await queueResponse
                        .text()
                        .catch(() => queueResponse.statusText);
                      throw new Error(
                        `Failed to fetch queue response (${queueResponse.status}): ${text.substring(0, 200)}`,
                      );
                    }
                    ...
                    console.log(
                      "[falService] ✅ Result fetched from queue URL:",
                      {
                        hasData: !!queueResultData?.data,
                        hasImages: !!(
                          queueResultData?.data &&
                    ...
                    if (
                      ...
                      console.warn(
                        "[falService] ⚠️ Result fetched from queue URL but no images found, continuing to poll...",
                      );
                      // Don't break - continue polling as result might appear later
                    }
                  } catch (queueUrlError: any) {
                    console.warn(
                      "[falService] Failed to fetch from queue URL:",
                      {
                        error: queueUrlError?.message,
                        status:
                          queueUrlError?.status || queueUrlError?.statusCode,
                      },
                    );
                  }
                }
                ...
                if (!foundResult) {
                  console.log(
                    "[falService] ⚠️ Result not found, will continue polling (attempt " +
                      (attempts + 1) +
                      "/" +
                      maxAttempts +
                      ")...",
                ...
                throw new ApiError(
                  `FAL queue generation failed: ${status?.error || status?.message || "Unknown error"}`,
                  502,
                );
              }
              ...
              if (attempts % 10 === 0) {
                ...
                console.warn(
                  "[falService] Queue polling error (will retry):",
                  pollError?.message,
                );
              }
            }
          ...
          if (!queueResult) {
            throw new ApiError(
              "FAL queue generation timed out after 10 minutes",
              504,
            );
          }
          ...
          result = await fal.subscribe(
            modelEndpoint as any,
            { input, logs: true } as unknown as any,
          );
        }
        ...
        if (result?.data?.images?.length > 0) {
          ...
          imageUrl =
            typeof firstOutput === "string"
              ? firstOutput
              : firstOutput?.url || firstOutput?.image_url;
        }

        if (!imageUrl) {
          console.error(
            "[falService] ❌ Failed to extract image URL from result:",
            {
        ...
        console.log(
          "[falService] ✅ Successfully extracted image URL from result:",
    ...
    const username = creator?.username || uid;
    const outputFolder =
      generationType === "text-to-character" ? "character" : "image";
    ...
    if (generationType === "text-to-character") {
        ...
        images.map(async (img, index) => {
          try {
            const { key, publicUrl } = await uploadFromUrlToZata({
              sourceUrl: img.url,
              keyPrefix:
                (payload as any)?.storageKeyPrefixOverride ||
                `users/${username}/${outputFolder}/${historyId}`,
            ...
            console.error(
              "[falService.generate] Zata upload failed for character:",
              e,
            );
            return {
              id: img.id,
      ...
      const scoredImages =
        await aestheticScoreService.scoreImages(storedImages);
      ...
      try {
        markGenerationCompleted(uid, historyId, {
          ...
          console.error(
            "[falService.generate] markGenerationCompleted (character) failed:",
            err,
          ),
        );
      } catch (optErr) {
        console.warn(
          "[falService.generate] markGenerationCompleted invocation error (character):",
          optErr,
        );
      }
      ...
      if (characterName && scoredImages.length > 0) {
        try {
          const { characterRepository } =
            await import("../repository/characterRepository");
          const generatedImage = scoredImages[0];
          const historyEntry = await generationHistoryRepository.get(
            uid,
            historyId,
          );
          ...
          console.error(
            "[falService.generate] Failed to save character:",
            charErr,
          );
          // Non-fatal
        }
      ...
      if (
        (payload as any)?.forceSyncUpload === true ||
        (payload as any)?.storageKeyPrefixOverride ||
        ...
        try {
          ...
          try {
            markGenerationCompleted(uid, historyId, {
              ...
              console.error(
                "[falService.generate] markGenerationCompleted (background upload) failed:",
                err,
              ),
            );
          } catch (optErr) {
            console.warn(
              "[falService.generate] markGenerationCompleted invocation error (background upload):",
              optErr,
            );
          }
        } catch (e) {
          console.error(
            "[falService.generate] Background Zata upload failed:",
            e,
          );
          try {
            await falRepository.updateGenerationRecord(legacyId, {
    ...
    try {
      ...
      console.error(
        "[falService.generate] Failed to mirror error state:",
        mirrorErr,
      );
    }
    throw falError;
...
async function veoTextToVideo(
  ...
  const { historyId } = await generationHistoryRepository.create(uid, {
    ...
    createdBy: creator
      ? { uid, username: creator.username, email: creator.email }
      : { uid },
  });
  ...
  try {
    ...
    if (ctx && ctx.creditCost && ctx.creditCost > 0) {
      try {
        ...
        console.log(
          `[falService.veoTextToVideo] Debited ${ctx.creditCost} credits for ${uid}`,
        );
      } catch (error) {
        console.error(
          "[falService.veoTextToVideo] Failed to deduct credits:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[veoTextToVideo] Failed to mirror error state:",
        mirrorErr,
      );
    }
    throw falError;
...
async function veoTextToVideoFast(
  uid: string,
  payload: Parameters<typeof veoTextToVideo>[1],
  ctx: any = {},
) {
  const falKey = env.falKey as string;
  if (!falKey) throw new ApiError("FAL AI API key not configured", 500);
  ...
  try {
    ...
    if (ctx && ctx.creditCost && ctx.creditCost > 0) {
      try {
        ...
        console.log(
          `[falService.veoTextToVideoFast] Debited ${ctx.creditCost} credits for ${uid}`,
        );
      } catch (error) {
        console.error(
          "[falService.veoTextToVideoFast] Failed to deduct credits:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[veoTextToVideoFast] Failed to mirror error state:",
        mirrorErr,
      );
    }
    throw falError;
...
async function veoImageToVideo(
  ...
  try {
    const result = await fal.subscribe(
      "fal-ai/veo3/image-to-video" as any,
      {
        input: {
          prompt: payload.prompt,
    ...
    if (ctx && ctx.creditCost && ctx.creditCost > 0) {
      try {
        ...
        console.log(
          `[falService.veoImageToVideo] Debited ${ctx.creditCost} credits for ${uid}`,
        );
      } catch (error) {
        console.error(
          "[falService.veoImageToVideo] Failed to deduct credits:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[veoImageToVideo] Failed to mirror error state:",
        mirrorErr,
      );
    }
    throw falError;
...
async function veoImageToVideoFast(
  uid: string,
  payload: Parameters<typeof veoImageToVideo>[1],
  ctx: any = {},
) {
  const falKey = env.falKey as string;
  if (!falKey) throw new ApiError("FAL AI API key not configured", 500);
  ...
  try {
    const result = await fal.subscribe(
      "fal-ai/veo3/fast/image-to-video" as any,
      {
        input: {
          prompt: payload.prompt,
          image_url: payload.image_url,
    ...
    if (ctx && ctx.creditCost && ctx.creditCost > 0) {
      try {
        ...
        console.log(
          `[falService.veoImageToVideoFast] Debited ${ctx.creditCost} credits for ${uid}`,
        );
      } catch (error) {
        console.error(
          "[falService.veoImageToVideoFast] Failed to deduct credits:",
          error,
        );
      }
    }
    ...
    try {
      ...
      console.error(
        "[veoImageToVideoFast] Failed to mirror error state:",
        mirrorErr,
      );
    }
    throw falError;
...
export const falService = {
  ...
  async briaExpandImage(
    ...
    if (!body?.image_url && !body?.image)
      throw new ApiError("image_url or image is required", 400);
    fal.config({ credentials: falKey });
    ...
    const promptText =
      typeof body?.prompt === "string" ? String(body.prompt) : "";
    const { historyId } = await generationHistoryRepository.create(uid, {
    ...
    try {
      ...
      if (!inputUrl && typeof body?.image === "string") {
        try {
          ...
          const stored = await uploadDataUriToZata({
            ...
            fileName: "bria-expand-source",
          });
          inputUrl = stored.publicUrl;
        } catch {}
      }
      if (!inputUrl)
        throw new ApiError("Unable to resolve image_url for Bria Expand", 400);
      ...
      if (isLocalhost || isZataUrl) {
        try {
          ...
          console.log(
            "[falService.briaExpandImage] Re-uploaded image for FAL access:",
            inputUrl.substring(0, 100) + "...",
          );
        } catch (err: any) {
          console.error(
            "[falService.briaExpandImage] Failed to re-upload image for FAL access:",
            err?.message || err,
          );
      ...
      // Normalize inputs
      const canvas =
        Array.isArray(body?.canvas_size) && body.canvas_size.length === 2
          ? [Number(body.canvas_size[0]), Number(body.canvas_size[1])]
          : undefined;
      const origSize =
        Array.isArray(body?.original_image_size) &&
        body.original_image_size.length === 2
          ? [
              Number(body.original_image_size[0]),
              Number(body.original_image_size[1]),
            ]
          : undefined;
      const origLoc =
        Array.isArray(body?.original_image_location) &&
        body.original_image_location.length === 2
          ? [
              Number(body.original_image_location[0]),
              Number(body.original_image_location[1]),
            ]
          : undefined;
      const aspect =
      ...
      console.log("[falService.briaExpandImage] Calling FAL API:", {
        model,
        input: {
          ...input,
      ...
      if (
        input.canvas_size &&
        input.original_image_size &&
        input.original_image_location
      ) {
        ...
        if (
          ...
          console.warn(
            "[falService.briaExpandImage] Image extends beyond canvas bounds:",
            {
      ...
      const imgUrl: string | undefined =
        ...
        (typeof (result as any)?.data?.image === "string"
      ...
      if (!imgUrl) {
        console.error(
          "[falService.briaExpandImage] No image in response. Full response:",
          JSON.stringify(result, null, 2),
        );
        throw new ApiError(
          "No image URL returned from FAL Bria Expand API. Check response structure.",
          502,
        );
      }
      ...
      if (imgUrl.startsWith("data:")) {
        ...
        const { key: uploadedKey, publicUrl: uploadedUrl } =
          await uploadDataUriToZata({
            dataUri: imgUrl,
            keyPrefix,
      ...
      const cleanedImages = scoredImages.map((img: any) => {
        ...
        if (typeof img.storagePath === "string")
          cleaned.storagePath = img.storagePath;
        if (typeof img.avifUrl === "string") cleaned.avifUrl = img.avifUrl;
        if (typeof img.thumbnailUrl === "string")
          cleaned.thumbnailUrl = img.thumbnailUrl;
        if (typeof img.blurDataUrl === "string")
          cleaned.blurDataUrl = img.blurDataUrl;
        if (typeof img.optimized === "boolean")
          cleaned.optimized = img.optimized;
        if (typeof img.optimizedAt === "string")
          cleaned.optimizedAt = img.optimizedAt;

        // Extract aestheticScore from aesthetic object if present, otherwise use direct value
        const aestheticScore =
          typeof img.aesthetic?.score === "number"
            ? img.aesthetic.score
            : typeof img.aestheticScore === "number"
              ? img.aestheticScore
              : undefined;
      ...
      console.log("[falService.briaExpandImage] Cleaned images:", {
        ...
        firstImage: cleanedImages[0]
          ? {
              ...
              hasAestheticScore:
                typeof cleanedImages[0].aestheticScore === "number",
            }
          : null,
      });
            ...
            if (
              ...
              value.forEach((item, index) => {
                if (typeof item === "object" && item !== null) {
                  issues.push(
                    `Array item at ${currentPath}[${index}] is an object`,
                  );
                  issues.push(
                    ...inspectForNestedStructures(
                      item,
                      `${currentPath}[${index}]`,
                    ),
                  );
      ...
      if (nestedIssues.length > 0) {
        console.error(
          "[falService.briaExpandImage] NESTED STRUCTURES DETECTED:",
          nestedIssues,
        );
        console.error(
          "[falService.briaExpandImage] Full cleaned images:",
          JSON.stringify(cleanedImages, null, 2),
        );
      } else {
        console.log(
          "[falService.briaExpandImage] ✅ No nested structures detected in cleaned images",
        );
      }
      ...
      console.log("[falService.briaExpandImage] Update payload to Firestore:", {
        ...
        imagesStructure: updatePayload.images.map((img, i) => ({
          ...
          types: Object.entries(img).map(([k, v]) => [
            k,
            typeof v,
      ...
      try {
        await generationHistoryRepository.update(
          uid,
          historyId,
          updatePayload as any,
        );
        console.log(
          "[falService.briaExpandImage] ✅ Successfully saved to Firestore",
        );
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.briaExpandImage] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.briaExpandImage] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[briaExpandImage] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  async outpaintImage(
    ...
    try {
      ...
      if (!resolvedUrl)
        throw new ApiError("Unable to resolve image_url for outpaint", 400);
      ...
      if (resolvedUrl && resolvedUrl.includes("idr01.zata.ai")) {
        try {
          ...
          console.log(
            "[falService.outpaintImage] Re-uploaded image for FAL access:",
            resolvedUrl,
          );
        } catch (err: any) {
          console.error(
            "[falService.outpaintImage] Failed to re-upload image for FAL access:",
            err?.message || err,
          );
      ...
      const zoomOut = Number.isFinite(zoomValue)
        ? Math.max(0, Math.min(100, zoomValue))
        : 20;
      ...
      const input: any = {
        ...
        sync_mode: syncMode,
        output_format: ["png", "jpeg", "jpg", "webp"].includes(outputFormat)
          ? outputFormat
          : "png",
      };
      if (promptText) input.prompt = promptText;
      if (
        typeof body?.aspect_ratio === "string" &&
        ["1:1", "16:9", "9:16", "4:3", "3:4"].includes(body.aspect_ratio)
      ) {
        input.aspect_ratio = body.aspect_ratio;
      }

      console.log("[falService.outpaintImage] Calling FAL API:", {
        model,
        input: {
          ...input,
      ...
      const endpointCandidates = [
        model,
        "fal-ai/image/outpaint",
        "fal-ai/outpaint/image",
      ];
      ...
      for (const endpoint of endpointCandidates) {
        try {
          if (endpoint !== model) {
            console.log(
              "[falService.outpaintImage] Retrying with endpoint:",
              endpoint,
            );
          }
          result = await fal.subscribe(
          ...
          console.error(
            "[falService.outpaintImage] FAL API error for",
            endpoint,
            ":",
          ...
          const isNotFound =
            msg.includes("not found") || falErr?.response?.status === 404;
          if (!isNotFound) {
            throw new ApiError(
              `FAL API error: ${JSON.stringify(details)}`,
              500,
            );
          }
        }
      ...
      const files: any[] = Array.isArray((result as any)?.data?.images)
        ? (result as any).data.images
        : [];
      if (!files.length) {
        console.error(
          "[falService.outpaintImage] No images in response:",
          JSON.stringify(result, null, 2),
        );
      ...
      const username = creator?.username || uid;
      const storedImages: FalGeneratedImage[] = await Promise.all(
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.outpaintImage] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.outpaintImage] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[outpaintImage] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
    ...
    try {
      ...
      if (resolvedUrl && /^data:/i.test(resolvedUrl)) {
        try {
          const stored = await uploadDataUriToZata({
            dataUri: resolvedUrl,
            keyPrefix: `users/${creator?.username || uid}/input/${historyId}`,
            fileName: "topaz-source",
          });
          resolvedUrl = stored.publicUrl;
        } catch {}
      } else if (resolvedUrl) {
        ...
        try {
          ...
          console.warn(
            "[topazUpscaleImage] Failed to upload input image to Zata, using original:",
            err,
          );
        }
      }
      if (!resolvedUrl)
        throw new ApiError(
          "Unable to resolve image_url for Topaz upscale",
          400,
        );
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.topazUpscaleImage] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.topazUpscaleImage] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[topazUpscaleImage] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
    ...
    try {
      ...
      if (resolvedUrl && /^data:/i.test(resolvedUrl)) {
        ...
        try {
          ...
          console.warn(
            "[seedvrUpscaleImage] Failed to upload input image to Zata, using original:",
            err,
          );
        }
      }
      ...
      if (!resolvedUrl)
        throw new ApiError(
          "Unable to resolve image_url for SeedVR image upscale",
          400,
        );
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.seedvrUpscaleImage] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.seedvrUpscaleImage] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[seedvrUpscaleImage] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
    ...
    try {
      ...
      if (resolvedUrl && resolvedUrl.includes("idr01.zata.ai")) {
        try {
          ...
          resolvedUrl = reuploaded.publicUrl;
          console.log(
            "[falService.seedvrUpscale] Re-uploaded video for FAL access:",
            resolvedUrl,
          );
        } catch (err: any) {
          console.error(
            "[falService.seedvrUpscale] Failed to re-upload video for FAL access:",
            err?.message || err,
          );
      ...
      console.log("[seedvrUpscale] Calling FAL API with input:", {
        ...input,
        video_url: input.video_url?.substring(0, 100) + "...",
      });

      // Validate that the video URL is an http(s) URL and reachable by FAL
      if (
        !input.video_url ||
        typeof input.video_url !== "string" ||
        !/^https?:\/\//i.test(input.video_url)
      ) {
        console.error(
          "[seedvrUpscale] Invalid video_url provided:",
          input.video_url,
        );
        throw new ApiError(
          "video_url must be a public http(s) URL accessible by the FAL service",
          400,
        );
      }
      ...
      try {
        ...
        if (
          ...
          console.warn(
            "[seedvrUpscale] video_url content-type is not a recognized video type:",
            contentType,
          );
        }
      } catch (headErr: any) {
        console.error(
          "[seedvrUpscale] Unable to reach video_url or non-200 response:",
          headErr?.message || headErr,
        );
        throw new ApiError(
          "Unable to download video_url; ensure it is a public, reachable URL (FAL needs to fetch it)",
          400,
        );
      }
      ...
      const videoUrl: string | undefined =
        ...
        (result as any)?.data?.output?.video?.url;
      if (!videoUrl) {
        console.error(
          "[seedvrUpscale] No video URL in response:",
          JSON.stringify(result, null, 2),
        );
        throw new ApiError("No video URL returned from FAL API", 502);
      }
      ...
      // Score video
      const scoredVideos = await aestheticScoreService.scoreVideos(
        videos as any,
      );
      const highestScore = aestheticScoreService.getHighestScore(scoredVideos);
      await generationHistoryRepository.update(uid, historyId, {
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.seedvrUpscale] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.seedvrUpscale] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[seedvrUpscale] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  async image2svg(
    ...
    try {
      ...
      if (!inputUrl)
        throw new ApiError("Unable to resolve image_url for image2svg", 400);
      ...
      try {
        ...
        if (
          inputUrl &&
          !inputUrl.includes(`users/${username}/input/${historyId}`)
        ) {
          try {
            const inputStored = await uploadFromUrlToZata({
              sourceUrl: inputUrl,
              keyPrefix: `users/${username}/input/${historyId}`,
              fileName: "vectorize-input",
            });
            inputImages = [
              {
                id: "in-1",
                url: inputStored.publicUrl,
                storagePath: inputStored.key,
            ...
            inputImages = [
              { id: "in-1", url: inputUrl, originalUrl: inputUrl },
            ];
          }
        } else if (inputUrl) {
      ...
      if (
        existing &&
        Array.isArray((existing as any).inputImages) &&
        (existing as any).inputImages.length > 0 &&
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.image2svg] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.image2svg] Failed to deduct credits:",
            error,
          );
        }
      }
  ...
  async recraftVectorize(
    ...
    try {
      ...
      if (!inputUrl)
        throw new ApiError("Unable to resolve image_url for vectorize", 400);

      const result = await fal.subscribe(
        model as any,
        { input: { image_url: inputUrl }, logs: true } as unknown as any,
      );
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.recraftVectorize] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.recraftVectorize] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[recraftVectorize] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  async briaGenfill(
    ...
    if (!body?.mask_url && !body?.mask)
      throw new ApiError("mask_url or mask is required", 400);
    ...
    try {
      ...
      if (!imageUrl && typeof body?.image === "string") {
        try {
          ...
          const stored = await uploadDataUriToZata({
            dataUri: body.image,
            keyPrefix: `users/${username}/input/${historyId}`,
            fileName: "genfill-source",
          });
          imageUrl = stored.publicUrl;
        } catch {}
      }
      if (!imageUrl)
        throw new ApiError("Unable to resolve image_url for Bria GenFill", 400);

      // Resolve mask URL
      let maskUrl: string | undefined =
        typeof body?.mask_url === "string" && body.mask_url.length > 0
          ? body.mask_url
          : undefined;
      if (!maskUrl && typeof body?.mask === "string") {
        try {
          ...
          const stored = await uploadDataUriToZata({
            dataUri: body.mask,
            keyPrefix: `users/${username}/input/${historyId}`,
            fileName: "genfill-mask",
          });
          maskUrl = stored.publicUrl;
        } catch {}
      }
      if (!maskUrl)
        throw new ApiError("Unable to resolve mask_url for Bria GenFill", 400);
      ...
      console.log("[falService.briaGenfill] Calling FAL API:", {
        model,
        input: {
          ...input,
          image_url: (input.image_url || "").slice(0, 100) + "...",
          mask_url: (input.mask_url || "").slice(0, 100) + "...",
        },
      });
      let result: any;
      try {
        ...
        const details = falErr?.response?.data || falErr?.message || falErr;
        console.error(
          "[falService.briaGenfill] FAL API error:",
          JSON.stringify(details, null, 2),
        );
        throw new ApiError(`FAL API error: ${JSON.stringify(details)}`, 500);
      }
      ...
      if (!imagesArray.length) {
        console.error(
          "[falService.briaGenfill] No images in response:",
          JSON.stringify(result, null, 2),
        );
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.briaGenfill] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.briaGenfill] Failed to deduct credits:",
            error,
          );
        }
      }
  ...
  async birefnetVideo(
    ...
    try {
      ...
      if (body.operating_resolution)
        input.operating_resolution = body.operating_resolution;
      if (typeof body.output_mask === "boolean")
        input.output_mask = body.output_mask;
      if (typeof body.refine_foreground === "boolean")
        input.refine_foreground = body.refine_foreground;
      if (body.sync_mode === true) input.sync_mode = true;
      if (body.video_output_type)
        input.video_output_type = body.video_output_type;
      ...
      try {
        ...
        const details = falErr?.response?.data || falErr?.message || falErr;
        console.error(
          "[birefnetVideo] FAL API error:",
          JSON.stringify(details, null, 2),
        );
        throw new ApiError(`FAL API error: ${JSON.stringify(details)}`, 502);
      }
      const videoUrl: string | undefined =
        (result as any)?.data?.video?.url || (result as any)?.data?.video_url;
      if (!videoUrl) {
        console.error(
          "[birefnetVideo] No video URL in response:",
          JSON.stringify(result, null, 2),
        );
        throw new ApiError("No video URL returned from FAL API", 502);
      }
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.birefnetVideo] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.birefnetVideo] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      try {
        ...
        console.error(
          "[birefnetVideo] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
  ...
  async qwenMultipleAngles(
    ...
    if (
      ...
      throw new ApiError(
        "image_urls is required and must be a non-empty array",
        400,
      );
    }
    fal.config({ credentials: falKey });
    ...
    const promptText =
      typeof body?.additional_prompt === "string"
        ? String(body.additional_prompt)
        : "";
    const { historyId } = await generationHistoryRepository.create(uid, {
      prompt: promptText
        ? `Qwen Multiple Angles: ${promptText}`
    ...
    try {
      ...
      console.log("[falService.qwenMultipleAngles] Calling FAL API:", {
        ...
        input: {
          ...input,
          image_urls: input.image_urls.map(
      ...
      try {
        ...
        const details = falErr?.response?.data || falErr?.message || falErr;
        console.error(
          "[falService.qwenMultipleAngles] FAL API error:",
          JSON.stringify(details, null, 2),
        );
        throw new ApiError(`FAL API error: ${JSON.stringify(details)}`, 500);
      }
      ...
      if (!imagesArray.length) {
        console.error(
          "[falService.qwenMultipleAngles] No images in response:",
          JSON.stringify(result, null, 2),
        );
        throw new ApiError(
          "No images returned from FAL Qwen Multiple Angles API",
          502,
        );
      }
      ...
      if (ctx && ctx.creditCost && ctx.creditCost > 0) {
        try {
          ...
          console.log(
            `[falService.qwenMultipleAngles] Debited ${ctx.creditCost} credits for ${uid}`,
          );
        } catch (error) {
          console.error(
            "[falService.qwenMultipleAngles] Failed to deduct credits:",
            error,
          );
        }
      }
      ...
      const falError = buildFalApiError(err, {
        fallbackMessage:
          "Failed to generate multiple camera angles with Qwen API",
      ...
      try {
        ...
        console.error(
          "[qwenMultipleAngles] Failed to mirror error state:",
          mirrorErr,
        );
      }
      throw falError;
...
async function persistInputImagesFromUrls(
  uid: string,
  historyId: string,
...
async function klingO1FirstLastSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
  const falKey = env.falKey as string;
  ...
  try {
    ...
    console.error(
      "[falService.klingO1FirstLastSubmit] Failed to upload images to Zata:",
      e,
    );
    throw new ApiError(
...
async function klingO1ReferenceSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
  const falKey = env.falKey as string;
  ...
  if (
    ...
    throw new ApiError(
      "image_urls array is required and must contain at least one image",
      400,
    );
  }
  ...
  try {
    ...
    console.error(
      "[falService.klingO1ReferenceSubmit] Failed to upload images to Zata:",
      e,
    );
    throw new ApiError(
      "Failed to upload images: " + (e?.message || String(e)),
...
async function queueStatus(
  ...
  if (!resolvedModel) {
    throw new ApiError(
      "Model is required. Either provide it in the request or ensure the requestId exists in generation history.",
      400,
    );
  }
  ...
  if (
    ...
    try {
      console.log(
        "[queueStatus] Job COMPLETED, triggering strict finalization/debit",
        { requestId, uid },
      );
...
const extractFalVideoUrl = (payload: any): { url?: string; id?: string } => {
  ...
  if (
    ...
    payload.output.startsWith("http")
  ) {
    return { url: payload.output };
  }

  if (Array.isArray(payload?.output)) {
    const stringUrl = payload.output.find(
    ...
    if (objWithUrl?.url) {
      return {
        url: objWithUrl.url as string,
        id: objWithUrl?.id as string | undefined,
      };
    }
  }
...
async function fetchFalQueueResponse(
  modelPath: string,
  requestId: string,
): Promise<any> {
  const normalizedModel = modelPath.startsWith("fal-ai/")
    ? modelPath
    : `fal-ai/${modelPath}`;
  ...
  if (!res.ok) {
    ...
    throw new ApiError(
      `Failed to fetch FAL queue response (${res.status}): ${text}`,
      res.status,
    );
  }
  return res.json();
...
async function queueResult(
  ...
  if (!extractedVideo.url && responseUrl) {
    try {
      ...
      console.warn("[fal.queueResult] Failed to fetch response_url fallback", {
        requestId,
        err,
      });
    }
  ...
  if (extractedVideo.url && located) {
    ...
    try {
      ...
      try {
        const { generateAndAttachThumbnail } =
          await import("./videoThumbnailService");
        const videoWithThumbnail = await generateAndAttachThumbnail(
          videoObj,
          keyPrefix,
        );
        ...
        console.warn(
          "[falService.queueResult] Failed to generate thumbnail, continuing without it:",
          thumbErr,
        );
        videos = [videoObj as any];
      }

      await generationHistoryRepository.update(uid, located.id, {
        status: "completed",
        videos,
        ...(providerVideoId ? { soraVideoId: providerVideoId } : {}),
      } as any);
    } catch (e) {
    ...
    const enrichedVideos = (
      fresh?.videos && Array.isArray(fresh.videos) ? fresh.videos : videos
    ).map((v: any) => ({
      id: v.id,
      url: v.url,
    ...
    try {
      ...
      console.error("[fal.queueResult] debit error", {
        uid,
        historyId: located.id,
        model: resolvedModel,
        err: e?.message || e,
  ...
  if (located && (result?.data?.images?.length || result?.data?.image?.url)) {
    ...
    const providerImages: { url: string }[] = Array.isArray(
      result?.data?.images,
    )
    ...
    const stored = await Promise.all(
      providerImages.map(async (img, index) => {
        try {
          const up = await uploadFromUrlToZata({
            sourceUrl: img.url,
            keyPrefix,
            fileName: buildGenerationImageFileName(located?.id, index),
          });
          return {
...
async function kling26ProT2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
  const falKey = env.falKey as string;
...
async function kling26ProI2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
  const falKey = env.falKey as string;
  ...
  try {
    ...
    console.error(
      "[falService.kling26ProI2vSubmit] Failed to upload image to Zata:",
      e,
    );
    // Continue with original URL if upload fails
  }
...
async function klingV3Submit(
  ...
  const multiPrompt = Array.isArray(body?.multi_prompt)
    ? body.multi_prompt
    : undefined;
  ...
  const username = creator?.username || uid;
  const { historyId } = await queueCreateHistory(uid, {
    prompt: prompt || "Multi-shot Kling 3 generation",
    model,
    isPublic: body.isPublic,
  });
  const keyPrefix = `users/${username}/input/${historyId}`;
  ...
  if (mode === "image-to-video") {
    ...
    try {
      if (typeof startImageUrl === "string" && /^data:/i.test(startImageUrl)) {
        const stored = await uploadDataUriToZata({
          dataUri: startImageUrl,
          keyPrefix,
          fileName: "start-image",
        });
      ...
      const persistedInputUrls = [startImageUrl, endImageUrl].filter(
      ...
      console.error(
        `[falService.klingV3Submit] Failed to upload ${tier} ${mode} image to Zata:`,
        e,
      );
    }
  }
...
async function klingV3StandardT2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
...
async function klingV3StandardI2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
...
async function klingV3ProT2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
...
async function klingV3ProI2vSubmit(
  uid: string,
  body: any,
): Promise<SubmitReturn> {
...
export const falQueueService = {
  ...
  async veo31I2vSubmit(
    ...
    try {
      ...
      console.error(
        "[falService.veo31I2vSubmit] Failed to upload image to Zata:",
        e,
      );
      throw new ApiError(
        "Failed to upload image: " + (e?.message || String(e)),
        500,
      );
    }
  ...
  async veo31LiteI2vSubmit(uid: string, body: any): Promise<SubmitReturn> {
    ...
    try {
      ...
      console.error(
        "[falService.veo31LiteI2vSubmit] Failed to upload image to Zata:",
        e,
      );
      throw new ApiError(
        "Failed to upload image: " + (e?.message || String(e)),
  ...
  async veo31ReferenceToVideoSubmit(
    ...
    if (!Array.isArray(body?.image_urls) || body.image_urls.length === 0)
      throw new ApiError(
        "image_urls is required and must contain at least one URL",
        400,
      );
    ...
    try {
      ...
      console.error(
        "[falService.veo31ReferenceToVideoSubmit] Failed to upload images to Zata:",
        e,
      );
      throw new ApiError(
        "Failed to upload images: " + (e?.message || String(e)),
  ...
  async veo31FirstLastFastSubmit(
    ...
    try {
      ...
      console.error(
        "[falService.veo31FirstLastFastSubmit] Failed to upload images to Zata:",
        e,
      );
      throw new ApiError(
        "Failed to upload images: " + (e?.message || String(e)),
  ...
  async veo31FirstLastSubmit(uid: string, body: any): Promise<SubmitReturn> {
    ...
    try {
      ...
      console.error(
        "[falService.veo31FirstLastSubmit] Failed to upload images to Zata:",
        e,
      );
      throw new ApiError(
        "Failed to upload images: " + (e?.message || String(e)),
  ...
  async sora2I2vSubmit(uid: string, body: any): Promise<SubmitReturn> {
    ...
    console.log("[sora2I2vSubmit] Submitting to FAL:", {
      model,
      input: {
        ...input,
        image_url: input.image_url?.substring(0, 100) + "...",
  ...
  async sora2ProI2vSubmit(uid: string, body: any): Promise<SubmitReturn> {
    ...
    console.log("[sora2ProI2vSubmit] Submitting to FAL:", {
      model,
      input: {
        ...input,
        image_url: input.image_url?.substring(0, 100) + "...",
  ...
  async sora2RemixV2vSubmit(uid: string, body: any): Promise<SubmitReturn> {
    ...
    if (!videoId && body?.source_history_id) {
      const src = await generationHistoryRepository.get(
        uid,
        String(body.source_history_id),
      );
      const stored =
        (src as any)?.soraVideoId ||
        (Array.isArray((src as any)?.videos) &&
          (src as any)?.videos[0]?.soraVideoId);
      if (stored) videoId = stored;
    }
    if (!videoId)
      throw new ApiError(
        "video_id or source_history_id (with stored soraVideoId) is required",
        400,
      );
    ...
    if (body?.source_history_id) {
      try {
        ...
        if (src) {
          ...
          const source_is_pro =
            String((src as any)?.model || "")
              .toLowerCase()
              .includes("/pro") ||
            String(source_resolution || "").toLowerCase() === "1080p";
          await generationHistoryRepository.update(uid, historyId, {
            source_history_id: String(body.source_history_id),
            source_duration,
            source_resolution,
            source_is_pro: String(!!source_is_pro),
          } as any);
        }
      } catch {}
    }
