    ...
    if (!genType) {
      ...
      if (requiresPrompt.has(genType)) {
        if (
          ...
          return next(
            new ApiError(
              `prompt is required for generationType ${genType}`,
              400,
            ),
          );
        }
...
export const validateFalMayaTts = [
  ...
  body("max_tokens")
    .optional()
    .isInt({ min: 1 })
    .withMessage("max_tokens must be an integer"),
  body("repetition_penalty")
    .optional()
    .isFloat({ min: 0.1 })
    .withMessage("repetition_penalty must be a number"),
  body("output_format")
    .optional()
    .isIn(["wav", "mp3"])
    .withMessage("output_format must be wav or mp3"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return next(new ApiError("Validation failed", 400, errors.array()));
...
export const validateFalChatterboxMultilingual = [
  body("text")
    ...
    .notEmpty()
    .isLength({ max: 2000 })
    .withMessage("text is required and must be <= 2000 characters"),
  body("voice").optional().isString().withMessage("voice must be a string"),
  body("custom_audio_language")
    .optional()
    .isIn([
      "english",
      "arabic",
    ...
    .withMessage(
      "custom_audio_language must be one of the allowed language codes",
    ),
  body("voice_file_name")
    .optional()
    .isString()
    .withMessage("voice_file_name must be a string if provided"),
  body("exaggeration")
    .optional()
    .isFloat({ min: 0.0, max: 2.0 })
    .withMessage("exaggeration must be between 0.0 and 2.0"),
  body("temperature")
    .optional()
    .isFloat({ min: 0.0, max: 2.0 })
  ...
  body("cfg_scale")
    .optional()
    .isFloat({ min: 0, max: 1 })
    .withMessage("cfg_scale must be between 0 and 1"),
  body("seed").optional().isInt().withMessage("seed must be an integer"),

  body("audio_url")
    .optional()
    .isString()
    .withMessage("audio_url must be a string URL if provided"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return next(new ApiError("Validation failed", 400, errors.array()));
    ...
    if (
      voice &&
      typeof voice === "string" &&
      (voice.startsWith("http://") || voice.startsWith("https://"))
    ) {
      if (!customAudioLanguage) {
        return next(
          new ApiError(
            "custom_audio_language is required when voice is a custom audio URL",
            400,
          ),
        );
      }
...
export const validateFalChatterboxSts = [
  body("source_audio_url")
    .isString()
    .notEmpty()
    .withMessage("source_audio_url is required"),
  body("target_voice")
    .optional()
    .isString()
    .withMessage("target_voice must be a string"),
  body("target_voice_audio_url")
    .optional()
    .isString()
    .withMessage("target_voice_audio_url must be a string"),
  body("high_quality_audio")
    .optional()
    .isBoolean()
    .withMessage("high_quality_audio must be boolean"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return next(new ApiError("Validation failed", 400, errors.array()));
...
export const validateFalElevenDialogue = [
  ...
  body("inputs.*.text")
    .isString()
    .notEmpty()
    .withMessage("each input must contain text")
    .isLength({ max: 1000 })
    .withMessage("each input text must be at most 1000 characters"),
...
export const validateFalElevenTts = [
  ...
  body("custom_audio_language")
    ...
    .withMessage("custom_audio_language must be one of the allowed values"),
  body("exaggeration")
    .optional()
    .isFloat({ min: 0.25, max: 2.0 })
    .withMessage("exaggeration must be between 0.25 and 2.0"),
  body("stability")
    .optional()
    .isFloat({ min: 0, max: 1 })
    .withMessage("stability must be between 0 and 1"),
  body("similarity_boost")
    .optional()
    .isFloat({ min: 0, max: 1 })
    .withMessage("similarity_boost must be between 0 and 1"),
  body("style")
    .optional()
    .isFloat({ min: 0, max: 1 })
  ...
  body("speed")
    ...
    .withMessage("speed must be between 0.5 and 2.5"), // Relaxed backend limit to allow UI 0.0-1.2 and potential future expansion
  body("temperature")
    .optional()
    .isFloat({ min: 0.05, max: 5.0 })
    .withMessage("temperature must be between 0.05 and 5.0"),
  body("cfg_scale")
    .optional()
    .isFloat({ min: 0.0, max: 1.0 })
    .withMessage("cfg_scale must be between 0.0 and 1.0"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      console.error(
        "[validateFalElevenTts] ❌ Validation failed:",
...
export const validateFalElevenSfx = [
  ...
  body("duration_seconds")
    .optional()
    .isFloat({ min: 0.5, max: 22 })
    .withMessage("duration_seconds must be between 0.5 and 22 seconds"),
  body("prompt_influence")
    .optional()
    .isFloat({ min: 0, max: 1 })
    .withMessage("prompt_influence must be between 0 and 1"),
  body("output_format")
    .optional()
    .isIn([
      "mp3_22050_32",
    ...
    .withMessage("output_format must be one of the allowed audio formats"),
  ...
  body("generationType")
    .optional()
    .isIn(["sfx"])
    .withMessage("generationType must be sfx"),
  body("model").optional().isString().withMessage("model must be a string"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
...
export const validateFalQueueStatus = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    const requestId =
      (req.query.requestId as string) || (req.body?.requestId as string);
...
export const validateFalQwenMultipleAngles = [
  body("image_urls")
    .isArray({ min: 1 })
    .withMessage("image_urls is required and must be a non-empty array"),
  body("image_urls.*")
    .isString()
    .notEmpty()
    .withMessage("Each image_urls item must be a non-empty string URL"),
  body("horizontal_angle")
    .optional()
    .isFloat()
    .withMessage("horizontal_angle must be a number"),
  body("vertical_angle")
    .optional()
    .isFloat()
    .withMessage("vertical_angle must be a number"),
  body("zoom")
    .optional()
    .isFloat({ min: 0, max: 10 })
    .withMessage("zoom must be between 0 and 10"),
  ...
  body("guidance_scale")
    ...
    .withMessage("guidance_scale must be between 0 and 20"),
  body("num_inference_steps")
    .optional()
    .isInt({ min: 1, max: 100 })
    .withMessage("num_inference_steps must be between 1 and 100"),
  body("acceleration")
    .optional()
    .isIn(["none", "regular"])
    .withMessage("acceleration must be none or regular"),
  body("negative_prompt")
    .optional()
    .isString()
    .withMessage("negative_prompt must be a string"),
  body("seed").optional().isInt().withMessage("seed must be an integer"),
  body("sync_mode")
    .optional()
    .isBoolean()
    .withMessage("sync_mode must be boolean"),
  body("enable_safety_checker")
    .optional()
    .isBoolean()
    .withMessage("enable_safety_checker must be boolean"),
  body("output_format")
    .optional()
    .isIn(["png", "jpeg", "webp"])
    .withMessage("output_format must be png, jpeg, or webp"),
  body("num_images")
    .optional()
    .isInt({ min: 1, max: 10 })
    .withMessage("num_images must be between 1 and 10"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return next(new ApiError("Validation failed", 400, errors.array()));
...
const validateKlingV3PromptFields = (bodyValue: any) => {
  const hasPrompt =
    typeof bodyValue?.prompt === "string" && bodyValue.prompt.trim().length > 0;
  const hasMultiPrompt =
    Array.isArray(bodyValue?.multi_prompt) && bodyValue.multi_prompt.length > 0;
...
export const validateFalKlingV3T2v = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    (req.body as any).duration = normalizeKlingV3Duration(
      (req.body as any)?.duration,
    );
    next();
  },
];
...
export const validateFalKlingV3I2v = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (
      !req.body?.start_image_url ||
      typeof req.body.start_image_url !== "string"
    ) {
      return next(
        new ApiError("start_image_url is required (alias: image_url)", 400),
      );
    }
    (req.body as any).duration = normalizeKlingV3Duration(
      (req.body as any)?.duration,
...
export const validateFalVeoTextToVideoFastSubmit =
  validateFalVeoTextToVideoFast;
export const validateFalVeo31LiteTextToVideoSubmit =
  validateFalVeo31LiteTextToVideo;
...
export const validateFalVeo31FirstLastFast = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    const hasFirst =
      typeof req.body?.start_image_url === "string" ||
      typeof req.body?.first_frame_url === "string";
    if (!hasFirst) {
      return next(
        new ApiError(
          "first_frame_url is required (alias: start_image_url)",
          400,
        ),
      );
    }
...
export const validateFalVeo31ReferenceToVideo = [
  ...
  body("image_urls")
    .isArray({ min: 1 })
    .withMessage("image_urls must be a non-empty array of URLs"),
...
export const validateFalKlingO1FirstLastSubmit = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (
      ...
      typeof req.body.last_frame_url === "string"
    ) {
    ...
    const hasFirst =
      typeof req.body?.start_image_url === "string" &&
      req.body.start_image_url.length > 0;
    const hasLast =
      typeof req.body?.end_image_url === "string" &&
      req.body.end_image_url.length > 0;

    if (!hasFirst) {
      return next(
        new ApiError(
          "start_image_url is required (alias: first_frame_url)",
          400,
        ),
      );
    }
    if (!hasLast) {
      return next(
        new ApiError("end_image_url is required (alias: last_frame_url)", 400),
      );
    }
...
export const validateFalKlingO1ReferenceSubmit = [
  ...
  body("image_urls.*")
    .isString()
    .withMessage("Each image_urls item must be a string"),
  body("duration")
    .optional()
    .custom((value) => {
      const str = typeof value === "string" ? value : String(value);
  ...
  body("aspect_ratio")
    .optional()
    .isIn(["16:9", "9:16", "1:1"])
    .withMessage('aspect_ratio must be "16:9", "9:16", or "1:1"'),
  body("elements")
    .optional()
    .isArray()
...
export const validateFalSora2I2v = [
  ...
  body("resolution")
    ...
    .isIn(["auto", "720p"])
    .withMessage("resolution must be auto or 720p"),
  body("aspect_ratio")
    .optional({ nullable: true, checkFalsy: false })
    .isIn(["auto", "16:9", "9:16"])
    .withMessage("aspect_ratio must be auto, 16:9, or 9:16"),
  body("duration")
    .optional({ nullable: true, checkFalsy: false })
    .custom((value) => {
  ...
  body("originalPrompt")
    .optional({ nullable: true, checkFalsy: false })
    .isString(), // Allow for history display
...
export const validateFalSora2ProI2v = [
  ...
  body("resolution")
    .optional({ nullable: true, checkFalsy: false })
    .isIn(["auto", "720p", "1080p"])
    .withMessage("resolution must be auto, 720p, or 1080p"),
  body("aspect_ratio")
    .optional({ nullable: true, checkFalsy: false })
    .isIn(["auto", "16:9", "9:16"])
    .withMessage("aspect_ratio must be auto, 16:9, or 9:16"),
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (!errors.isEmpty()) {
      console.error(
        "[validateFalSora2ProI2v] Validation errors:",
        errors.array(),
      );
      return next(new ApiError("Validation failed", 400, errors.array()));
    }
...
export const validateFalVeo31FirstLast = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (!hasFirst) {
      return next(
        new ApiError(
          "first_frame_url is required (aliases: start_image_url)",
          400,
        ),
      );
    }
...
const validateFalLtx2I2vBase = [
  ...
  body("duration")
    .optional()
    .isIn([6, 8, 10])
    .withMessage("duration must be 6, 8, or 10"),
...
export const validateFalSora2T2v = [
  ...
  body("resolution")
    .optional({ nullable: true, checkFalsy: false })
    .isIn(["720p"])
    .withMessage("resolution must be 720p"),
  body("aspect_ratio")
    .optional({ nullable: true, checkFalsy: false })
    .isIn(["16:9", "9:16"])
    .withMessage("aspect_ratio must be 16:9 or 9:16"),
  body("duration")
    .optional({ nullable: true, checkFalsy: false })
    .custom((value) => {
      // Accept both number and string, but validate the value
...
export const validateFalSora2ProT2v = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (!errors.isEmpty()) {
      console.error(
        "[validateFalSora2ProT2v] Validation errors:",
        errors.array(),
      );
      return next(new ApiError("Validation failed", 400, errors.array()));
    }
...
export const validateFalSora2Remix = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    const hasVideoId =
      typeof req.body?.video_id === "string" && req.body.video_id.length > 0;
    const hasSource =
      typeof req.body?.source_history_id === "string" &&
      req.body.source_history_id.length > 0;
    if (!hasVideoId && !hasSource) {
      return next(
        new ApiError("Either video_id or source_history_id is required", 400),
      );
    }
    const errors = validationResult(req);
    if (!errors.isEmpty())
...
export const validateFalSora2RemixByHistory = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    if (req.body?.video_id) {
      return next(
        new ApiError(
          "Do not provide video_id on this route; it resolves from source_history_id",
          400,
        ),
      );
    }
...
const validateFalLtx2T2vBase = [
  ...
  body("aspect_ratio")
    .optional()
    .isIn(["16:9"])
    .withMessage("Only 16:9 is supported"),
  body("duration")
    .optional()
    .isIn([6, 8, 10])
    .withMessage("duration must be 6, 8, or 10"),
...
export const validateFalImage2Svg = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    const hasUrl =
      typeof req.body?.image_url === "string" && req.body.image_url.length > 0;
    const hasImage =
      typeof req.body?.image === "string" && req.body.image.length > 0;
    if (!hasUrl && !hasImage)
...
export const validateFalBriaExpand = [
  ...
  body("aspect_ratio")
    .optional()
    .isIn(["1:1", "2:3", "3:2", "3:4", "4:3", "4:5", "5:4", "9:16", "16:9"]),
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    if (
      Array.isArray(req.body?.canvas_size) &&
      req.body.canvas_size.length === 2
    ) {
      ...
      if (
        ...
        return next(
          new ApiError("canvas_size must be [width,height] each <= 5000", 400),
        );
      }
    }
    const errors = validationResult(req);
...
export const validateFalBriaGenfill = [
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    const hasMaskUrl =
      typeof req.body?.mask_url === "string" && req.body.mask_url.length > 0;
    const hasMask =
      typeof req.body?.mask === "string" && req.body.mask.length > 0;
...
export const validateFalSeedvrUpscale = [
  ...
  body("video_url")
    ...
    .custom((value, { req }) => {
      const hasVideo =
        typeof (req.body as any)?.video === "string" &&
        String((req.body as any).video).startsWith("data:");
  ...
  body("video")
    .optional()
    .isString()
    .withMessage("video must be a string if provided"), // allow data URI video as fallback
  body("upscale_mode")
    .optional()
    .isIn(["target", "factor"])
    .withMessage("upscale_mode must be target or factor"),
  body("upscale_factor")
    .optional()
    .isFloat({ gt: 0.1, lt: 10 })
    .withMessage("upscale_factor must be between 0.1 and 10"),
  body("target_resolution")
    .optional()
    .isIn(["720p", "1080p", "1440p", "2160p"]),
  body("seed").optional().isInt(),
  body("noise_scale").optional().isFloat({ min: 0, max: 2 }),
  body("output_format")
    .optional()
    .isIn(["X264 (.mp4)", "VP9 (.webm)", "PRORES4444 (.mov)", "GIF (.gif)"]),
  ...
  async (req: Request, _res: Response, next: NextFunction) => {
    ...
    const hasVideoUrl =
      typeof req.body?.video_url === "string" &&
      req.body.video_url.trim().length > 0;
    const hasVideoData =
      typeof (req.body as any)?.video === "string" &&
      String((req.body as any).video).startsWith("data:");
    ...
    if (!hasVideoUrl && !hasVideoData) {
      return next(
        new ApiError("Either video_url or video (data URI) is required", 400),
      );
    }
    ...
    if (hasVideoData && !hasVideoUrl) {
      try {
        ...
        return next(
          new ApiError("Failed to upload video data URI to storage", 400),
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[validateFalSeedvrUpscale] Video probe failed:",
      ...
      if (!isFinite(duration) || duration <= 0) {
        console.warn(
          "[validateFalSeedvrUpscale] Could not read video duration, but continuing - FAL will validate",
        );
      ...
      console.error(
        "[validateFalSeedvrUpscale] Validation error:",
        e?.message || e,
      );
...
export const validateFalBirefnetVideo = [
  ...
  body("operating_resolution")
    .optional()
    .isIn(["1024x1024", "2048x2048", "2304x2304"]),
  body("output_mask").optional().isBoolean(),
  body("refine_foreground").optional().isBoolean(),
  body("sync_mode").optional().isBoolean(),
  body("video_output_type")
    .optional()
    .isIn(["X264 (.mp4)", "VP9 (.webm)", "PRORES4444 (.mov)", "GIF (.gif)"]),
...
export const validateFalNanoBananaPro = [
  ...
  body("num_images")
    .optional()
    .isInt({ min: 1 })
    .withMessage("num_images must be an integer >= 1"),
  body("aspect_ratio")
    .optional()
    .isIn([
      "auto",
    ...
    .withMessage("aspect_ratio must be one of the allowed values"),
  body("output_format")
    .optional()
    .isIn(["jpeg", "png", "webp"])
    .withMessage("output_format must be jpeg, png, or webp"),
  body("sync_mode")
    .optional()
    .isBoolean()
    .withMessage("sync_mode must be boolean"),
  ...
  body("image_urls.*")
    .optional()
    .isString()
    .withMessage("image_urls must contain strings"),
  body("resolution")
    .optional()
    .isIn(["1K", "2K", "4K"])
    .withMessage("resolution must be 1K, 2K, or 4K"),
  body("limit_generations")
    .optional()
    .isBoolean()
    .withMessage("limit_generations must be boolean"),
  body("enable_web_search")
    .optional()
    .isBoolean()
    .withMessage("enable_web_search must be boolean"),
  (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
      return next(new ApiError("Validation failed", 400, errors.array()));
    ...
    const hasImageUrls =
      Array.isArray(req.body?.image_urls) && req.body.image_urls.length > 0;
    if (
      !hasImageUrls &&
      (!req.body?.prompt ||
        typeof req.body.prompt !== "string" ||
      ...
      return next(
        new ApiError("prompt is required for text-to-image generation", 400),
      );
    }
    ...
    if (hasImageUrls) {
      for (const url of req.body.image_urls) {
        if (typeof url !== "string" || url.trim().length === 0) {
          return next(
            new ApiError("image_urls must contain non-empty URL strings", 400),
          );
        }
      }
    }
    ...
    if (typeof url === "string" && url.length > 0) {
      try {
        ...
        if (!isFinite(w) || !isFinite(h) || w <= 0 || h <= 0) {
          return next(
            new ApiError(
              "Unable to read image dimensions. Ensure the URL is public and accessible.",
              400,
            ),
          );
        }
        ...
        return next(
          new ApiError("Failed to validate image URL for Topaz upscale", 400),
        );
      }
...
export const validateFalSeedvrUpscaleImage = [
  ...
  body("upscale_mode")
    .optional()
    .isIn(["factor"])
    .withMessage("upscale_mode must be factor"),
  // Explicitly forbid target mode fields
  body("target_resolution")
    .not()
    .exists()
    .withMessage("target_resolution is not supported"),
  body("upscale_factor")
    .optional()
    .isFloat({ gt: 0.1, lt: 10 })
    .withMessage("upscale_factor must be between 0.1 and 10"),
  body("noise_scale")
    .optional()
    .isFloat({ min: 0, max: 2 })
    .withMessage("noise_scale must be between 0 and 2"),
  body("output_format")
    .optional()
    .isIn(["jpg", "png", "webp"])
    .withMessage("output_format must be jpg, png, or webp"),
  body("seed").optional().isInt().withMessage("seed must be an integer"),
  async (req: Request, _res: Response, next: NextFunction) => {
    const errors = validationResult(req);
    if (!errors.isEmpty())
    ...
    const hasDataUri =
      typeof dataImage === "string" && dataImage.startsWith("data:");
    ...
    if (hasUrl) {
      try {
        ...
        return next(
          new ApiError(
            "Failed to validate image URL for SeedVR image upscale",
            400,
          ),
        );
      }
