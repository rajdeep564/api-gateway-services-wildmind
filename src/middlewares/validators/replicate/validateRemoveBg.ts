...
export function validateRemoveBg(req: Request, _res: Response, next: NextFunction) {
  ...
  if (isEraser) {
    ...
    if (!hasImage) return next(new ApiError('image or image_url is required for bria/eraser', 400));
  } else {
    if (!image || typeof image !== 'string') return next(new ApiError('image is required (url or data URI)', 400));
  }
  ...
  if (isEraser) {
    if (mask != null && typeof mask !== 'string') return next(new ApiError('mask must be a string (url or data URI)', 400));
    if (mask_url != null && typeof mask_url !== 'string') return next(new ApiError('mask_url must be a string (url)', 400));
    if (image_url != null && typeof image_url !== 'string') return next(new ApiError('image_url must be a string (url)', 400));
    if (mask_type != null && !['manual', 'automatic'].includes(String(mask_type).toLowerCase())) return next(new ApiError('mask_type must be "manual" or "automatic"', 400));
    if (preserve_alpha != null && typeof preserve_alpha !== 'boolean') return next(new ApiError('preserve_alpha must be boolean', 400));
    if (content_moderation != null && typeof content_moderation !== 'boolean') return next(new ApiError('content_moderation must be boolean', 400));
    if (sync != null && typeof sync !== 'boolean') return next(new ApiError('sync must be boolean', 400));
  }
