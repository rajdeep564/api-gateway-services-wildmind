...
// Validator for Replicate model: Seedance 1.0 Pro Fast T2V
...
//           image (string URI, for I2V mode only - NOT first/last frame support)
...
export const validateSeedanceProFastT2V = [
  ...
  // image is optional for T2V, but required for I2V (handled in I2V validator)
  body('image').optional().isString().isLength({ min: 5 }).withMessage('image must be a valid URI'),
  // Pro Fast does NOT support first_frame_image or last_frame_image
  ...
  (req: Request, _res: Response, next: NextFunction) => {
    ...
    // Validate that first_frame_image is not provided (not supported by Pro Fast)
    if (req.body.image && String(req.body.image).length > 5 && req.body.generationType === 'text-to-video') {
      // For T2V, image should not be provided (it's only for I2V)
      // But we allow it to be passed through for I2V mode
    }
