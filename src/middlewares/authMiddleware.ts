  ...
  try {
    ...
    if (env.logLevel === "debug" || req.headers["x-debug-auth"] === "true") {
      ...
      console.log("[AUTH][requireAuth] Cookie debug info:", {
        ...
        note:
          hasAppSessionInHeader && !token
            ? "Cookie exists in header but not parsed - check cookie parsing"
            : !hasAppSessionInHeader
              ? "Cookie NOT in request header - cookie not being sent from browser"
    ...
    if (!token) {
      ...
      console.error("[AUTH][requireAuth] 401 Unauthorized - No token", {
        ...
        possibleCauses: [
          !hasAppSessionInHeader
            ? "Cookie not being sent from browser (check cookie domain)"
            : "Cookie in header but not parsed",
  ...
  try {
    ...
    try {
      ...
      if (cached?.uid) {
        ...
        if (!isExpired) {
          // SECURITY: Verify the JWT's own uid claim matches what's in the cache.
          // Firebase JWTs embed the UID in the 'uid', 'sub', or 'user_id' field.
          // If they differ, the cache entry is stale/corrupt — discard and re-verify.
          const tokenPayload = decodeJwtPayload(token);
          const tokenUid =
            tokenPayload?.uid || tokenPayload?.sub || tokenPayload?.user_id;

          if (tokenUid && tokenUid !== cached.uid) {
            // UID mismatch — this is the identity mixup bug. Log as critical and force re-verify.
            console.error(
              "[AUTH][SECURITY] ⚠️ IDENTITY MIXUP PREVENTED: Cache UID does not match token UID. Invalidating stale cache entry and re-verifying with Firebase.",
              {
                cachedUid: cached.uid,
                tokenUid,
                tokenPrefix: token.substring(0, 20),
                note: "This indicates a stale Redis cache entry. The request will be re-verified with Firebase Admin.",
              },
            );
            await deleteCachedSession(token);
            // Fall through to Firebase verification below
          } else {
            // UID matches (or JWT had no uid claim to check) — safe to use cache
            if (env.redisDebug) {
              console.log("[AUTH][Redis] HIT", {
                uid: cached.uid,
                uidVerified: !!tokenUid,
              });
            }
            req.uid = cached.uid;
            req.authMethod = "cached";
            req.verifiedAuthToken = token;
            return next();
          }
        } else {
          // Cache says expired, verify with Firebase to be sure
          if (env.redisDebug) {
            console.log(
              "[AUTH][Redis] Cached session expired, verifying with Firebase",
              {
                uid: cached.uid,
      ...
      if (env.logLevel === "debug") {
        console.warn(
          "[AUTH] Cache lookup failed, falling back to token verification:",
          cacheError,
        );
      }
    }
    ...
    try {
      ...
      if (tokenType === "idToken") {
        console.warn(
          "[AUTH] ⚠️ Detected ID token in cookie (should be session cookie), verifying as ID token",
          {
            tokenPrefix: token.substring(0, 20),
        ...
        try {
          ...
          if (env.logLevel === "debug") {
            console.log("[AUTH] ID token verified (from cookie)", {
              uid: decoded.uid,
            });
          }
          ...
          try {
            ...
            const maxAgeInSeconds = Math.floor(
              SESSION_COOKIE_DURATION_MS / 1000,
            );
            const expirationDate = new Date(
              Date.now() + SESSION_COOKIE_DURATION_MS,
            );
            ...
            console.log(
              "[AUTH] ✅ Automatically replaced ID token with proper session cookie",
              {
                uid: decoded.uid,
            ...
            decoded = await admin
              .auth()
              .verifySessionCookie(sessionCookie, env.authStrictRevocation);
            ...
            console.warn(
              "[AUTH] Failed to create session cookie from ID token (non-fatal)",
              {
          ...
          const isExpired =
            idTokenVerifyError?.code === "auth/id-token-expired" ||
          ...
          if (isExpired) {
            ...
            console.error(
              "[AUTH] ❌ ID token is expired, cannot create session cookie",
              {
                tokenPrefix: token.substring(0, 20),
            ...
            throw new ApiError(
              "Unauthorized - ID token has expired. Please refresh your authentication.",
              401,
            );
          } else {
        ...
        isSessionCookie = true;
        if (env.logLevel === "debug") {
          console.log("[AUTH] Session cookie verified (unknown type)", {
            uid: decoded.uid,
          });
        }
      }
      ...
      const isIssuerMismatch =
        ...
        (sessionError?.message &&
          sessionError.message.includes("Expected") &&
          sessionError.message.includes("but got"));

      // If it's an issuer mismatch, immediately try ID token verification
      if (isIssuerMismatch) {
        console.warn(
          "[AUTH] Session cookie verification failed due to issuer mismatch, trying ID token verification",
          {
        ...
        try {
          ...
          isSessionCookie = false;
          if (env.logLevel === "debug") {
            console.log("[AUTH] ID token verified (after issuer mismatch)", {
              uid: decoded.uid,
            });
          }
          ...
          if (!req.headers.authorization && req.cookies?.[COOKIE_NAME]) {
            try {
              ...
              res.setHeader("Set-Cookie", setCookieValue);
              console.log(
                "[AUTH] ✅ Created session cookie from ID token (after issuer mismatch)",
                {
                  uid: decoded.uid,
                },
              );
              ...
              console.warn(
                "[AUTH] Failed to create session cookie (non-fatal)",
                {
                  error: createError?.message,
                },
              );
          ...
          if (isExpired) {
            console.error(
              "[AUTH] ❌ ID token is expired (from issuer mismatch fallback)",
              {
                tokenPrefix: token.substring(0, 20),
          ...
          // Both failed
          throw new ApiError(
            `Unauthorized - Token verification failed: ${idTokenError?.message || sessionError?.message}`,
            401,
          );
        }
      } else {
        ...
        const isRevocationError =
          sessionError?.code === "auth/session-cookie-revoked" ||
        ...
        if (isRevocationError && env.authStrictRevocation) {
          try {
            ...
            if (env.logLevel === "debug") {
              console.warn(
                "[AUTH] Session cookie verified without strict revocation (revocation check failed)",
                {
                  uid: decoded.uid,
                  error: sessionError?.message,
                },
        ...
        if (!decoded) {
          try {
            ...
            console.error(
              "[AUTH] Token verification failed - potential logout cause",
              {
            ...
            if (sessionError?.code === "auth/session-cookie-revoked") {
              console.error(
                "[AUTH] ⚠️ SESSION REVOKED ⚠️ - The session cookie was explicitly revoked by Firebase.",
              );
            }
    ...
    req.uid = decoded.uid;
    req.authMethod = isSessionCookie ? "session" : "idToken";
    req.verifiedAuthToken = token;
    ...
    try {
      ...
      const issuedAt =
        typeof decoded?.iat === "number" ? decoded.iat : undefined;
      ...
      console.warn(
        "[AUTH] Failed to cache session after verification (non-fatal but may impact performance):",
        {
          error: cacheError,
          uid: decoded?.uid,
          timestamp: new Date().toISOString(),
