...
export async function computeReplicateBgRemoveCost(
  req: Request,
): Promise<{
  cost: number;
  ...
  if (normalized.includes("bria/eraser")) {
    ...
    return {
      ...
      meta: { model: normalized, note: "Hardcoded Bria Eraser cost" },
    };
  } else if (normalized.includes("851-labs/background-remover"))
    display = "851-labs/background-remover";
...
export async function computeReplicateUpscaleCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
  ...
  if (normalized.includes("crystal")) {
    ...
    if (!inW || !inH || inW <= 0 || inH <= 0) {
      throw new ApiError(
        "Unable to determine image dimensions for Crystal Upscaler pricing",
        400,
      );
    }
    ...
    display = "replicate/nightmareai/real-esrgan";
  else if (normalized.includes("mv-lab/swin2sr"))
    display = "replicate/mv-lab/swin2sr";
  else display = "replicate/philz1337x/clarity-upscaler"; // Fallback

  const base = findCredits(display);
  if (base == null)
    throw new Error(`Unsupported Replicate Upscale model: ${display}`);
...
export async function computeReplicateImageGenCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
  ...
  const isRecraftV4 =
    normalized.includes("recraft-ai/recraft-v4") || normalized === "recraft-v4";
  ...
  if (
    ...
    normalized.includes("leonardoai/lucid-origin") ||
    normalized.includes("lucid origin") ||
    normalized.includes("lucid-origin")
  ) {
    display = "Lucid Origin";
  } else if (
    normalized.includes("phoenix 1.0") ||
    normalized.includes("phoenix-1.0")
  ) {
    display = "Phoenix 1.0";
  } else if (
    normalized.includes("p-image") ||
    normalized.includes("prunaai/p-image")
  ) {
    display = "P-Image";
  } else if (
    normalized.includes("p-image-edit") ||
    normalized.includes("prunaai/p-image-edit")
  ) {
    ...
    display = normalized.includes("2512")
  ...
  const base = findCredits(display);
  if (base == null)
    throw new Error(`Unsupported Replicate Image model: ${display}`);
...
export async function computeReplicateMultiangleCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
...
export async function computeReplicateNextSceneCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
...
export async function computeQwenImageEditCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
  ...
  if (requested.includes("qwen/qwen-image-2-pro")) {
    ...
    display = requested.includes("2512")
      ? "qwen-image-edit-2512"
      : "qwen-image-edit-2511";
  }
...
export async function computeReplicateVideoCost(
  req: Request,
): Promise<{
  cost: number;
  pricingVersion: string;
  ...
  if (base == null) {
    console.warn(
      `[computeReplicateVideoCost] Pricing not found for "${display}", using fallback 760`,
    );
    return {
      cost: 760,
      pricingVersion: REPLICATE_PRICING_VERSION,
      meta: { ...meta, display, note: "Fallback pricing" },
    };
  }
