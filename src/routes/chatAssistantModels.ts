...
import { creditsRepository, getModelCost } from '../repository/creditsRepository';
...
import { assistantThreadsRepository, AssistantAttachment } from '../repository/assistantThreadsRepository';
import { CHAT_MODEL_CONFIGS, CHAT_MODE_MODEL_IDS, isChatModeModelId } from '../config/assistantModels';
...
function normalizeAttachments(value: unknown): AssistantAttachment[] {
    ...
    return value
        .filter((item): item is AssistantAttachment => !!item && typeof item === 'object' && typeof (item as any).url === 'string')
        .map((item) => ({
            id: String(item.id || `assistant-att-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`),
            type: item.type,
            url: item.url,
...
function serializeMessageForContext(content: string, attachments: AssistantAttachment[]): string {
    const sanitized = String(content || '').trim();
    if (attachments.length === 0) return sanitized;

    const attachmentSummary = attachments
        .map((attachment) => `${attachment.type}${attachment.fileName ? `:${attachment.fileName}` : ''}`)
        .join(', ');

    return `${sanitized}\n[attachments: ${attachmentSummary}]`.trim();
}

function buildConversationHistoryFromPersisted(messages: Array<{
    role: 'user' | 'assistant';
    content: string;
...
function mergeGeminiInputWithAttachments(
    ...
    const imageUrls = attachments.filter((item) => item.type === 'image').map((item) => item.url);
    const videoUrls = attachments.filter((item) => item.type === 'video').map((item) => item.url);
    const audioUrl = attachments.find((item) => item.type === 'audio')?.url ?? null;

    if (!modelInput && imageUrls.length === 0 && videoUrls.length === 0 && !audioUrl) {
        return undefined;
    }
...
function validateGeminiAttachments(attachments: AssistantAttachment[]): string | null {
    ...
    const oversizedImage = attachments.find(
        (item) => item.type === 'image' && typeof item.sizeBytes === 'number' && item.sizeBytes > GEMINI_MAX_IMAGE_BYTES
    );
    if (oversizedImage) {
...
router.post('/', requireAuth, async (req, res) => {
    try {
        ...
        const { message, history = [], modelId, modelInput, threadId, attachments = [] } = req.body as {
            message: string;
            history?: Array<{ role: 'user' | 'assistant'; content: string }>;
            modelId?: string;
            modelInput?: GeminiChatModeInput;
            threadId?: string;
            attachments?: AssistantAttachment[];
        };
        ...
        if (activeThread.mode !== 'chat') {
            return res.status(400).json(
                formatApiResponse('error', 'Chat route can only be used with chat threads', null)
            );
        }

        if (activeThread.modelId !== modelId) {
            return res.status(400).json(
                formatApiResponse('error', 'This thread is locked to a different model', {
        ...
        if (selectedModelId === 'google/gemini-3.1-pro') {
            const attachmentValidationError = validateGeminiAttachments(normalizedAttachments);
            if (attachmentValidationError) {
                return res.status(400).json(
                    formatApiResponse('error', attachmentValidationError, null)
                );
            }
        }
        const effectiveModelInput = selectedModelId === 'google/gemini-3.1-pro'
            ? mergeGeminiInputWithAttachments(modelInput, normalizedAttachments)
            : undefined;
        const persistedMessages = await assistantThreadsRepository.listMessages(uid, activeThread.id, 20);
        const conversationHistory = persistedMessages.length > 0
        ...
        const validationPricingParams = getAssistantChatValidationPricingParams(
            ...
            sanitized,
            conversationHistory,
            selectedModelId === 'google/gemini-3.1-pro' ? effectiveModelInput : undefined
        );
        const validationCost = await resolveChatModeCost(selectedModelId, validationPricingParams);
        ...
        const reply = await generateAssistantChatModeResponse({
            ...
            geminiInput: selectedModelId === 'google/gemini-3.1-pro' ? effectiveModelInput : undefined,
        });
        ...
        const finalCost = await resolveChatModeCost(selectedModelId, finalPricingParams);
        ...
        await creditsRepository.writeDebitIfAbsent(
            ...
            'Assistant Chat Mode',
            {
                ...
                modelInput: selectedModelId === 'google/gemini-3.1-pro' ? effectiveModelInput : undefined,
            ...
            messages: [
                {
                    ...
                    modelInput: selectedModelId === 'google/gemini-3.1-pro' ? effectiveModelInput ?? null : null,
                    metadata: {
                        mode: 'chat',
                        modelId: selectedModelId,
                    },
        ...
        return res.status(500).json(
            formatApiResponse('error', 'Assistant chat mode is temporarily unavailable. Please try again.', null)
        );
    }
});
